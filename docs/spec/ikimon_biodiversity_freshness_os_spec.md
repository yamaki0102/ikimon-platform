...
| **curator_run** | CMA agent の 1 回の実行。複数 snapshot を入力に複数 version 提案を出力 |
...
[Versioned Biodiversity Knowledge Store]
   ...
   全テーブルが source_snapshot_id FK + valid_from/valid_to を持つ
                          ↓
[AI Curator Agents] (Claude Managed Agents)
   差分要約 / 矛盾検出 / claim 抽出 / 影響範囲算出
   出力は ai_curator_runs.proposed_changes に格納
...
| **Warm** | systemd timer + webhook | Claude Managed Agent + MCP | 数分〜数十分 | `source_snapshots`, `*_versions` (PR経由), `ai_curator_runs`, `claim_review_queue` |
...
### 4.4 ai_curator_runs
CMA agent 実行ログ。`proposed_changes JSONB` に「どの table にどの行を提案したか」を構造化。`pr_url` で人手承認の追跡。
...
仮定: Flash Lite $0.10/M 入力 / $0.40/M 出力。1 req = 入力 4k + 出力 1.2k ≈ **0.13 円/req**。

| シナリオ | req/月 | 備考 |
|---|---|---|
| cache 0% | 76,000 | 理論最大 |
| cache 50% (現実値想定) | 150,000 | profile_digest 不変前提 |
| Pro 5% 混在 | 60,000 | 安全係数込み |
...
3. CMA agent (Sonnet 4.7 系) を MCP 経由で起動。input snapshot_ids を渡す
4. agent が proposed_changes を生成 → `ai_curator_runs.proposed_changes` に保存
5. `out/proposals/<run_id>.sql` を出力 (DB DDL/DML)
6. GitHub Action `agent-curator-pr.yml` が PR 化 + `agent-generated` ラベル付与
...
| リスク | 検出シグナル | 撤退アクション |
|---|---|---|
| CMA ベータ撤退/breaking | curator 連続 3 回失敗 | Anthropic Messages API 直叩き worker (`mcp_servers/ikimon-db-mcp/standalone-runner.ts`) にフォールバック。プロンプト md は流用 |
...
| Xserver VPS リソース枯渇 | systemd OOM | curator は trigger のみ、本体 CMA クラウド側。audio embedding batch_size を ENV 化 |
...
- 期待: P95 < 1.5s、`ai_cost_log` の 1 req コスト 0.13 円 ± 30%、cache hit ratio ≥ 50%
