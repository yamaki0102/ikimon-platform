...
この節は画面・文言・状態の設計を定める。設計の採用と、画面の実装・実利用の確認は別である。設計のみの整理後、2026-09-05にownerが「チェック問題ないなら反映して本番に」と実装・公開を承認した。今回の反映範囲と残る実装上の差は§16.9に記す。判断理由は [ADR-0001](decisions/ADR-0001-read-first-and-explicit-state-design.md) に置く。
...
実装の参照は `src/ui/appExperience.ts`、`siteShell.ts`、`landingHomeState.ts`、`mapExplorer.ts`、Workerのnative record/list/detail/event/profile renderer。共有するのは依存のないナビと表示規則だけで、権限・公開判断・データ保管を新しく統合しない。

- 共通ナビはPCのヘッダーと1160px以下の下部ナビを切り替える。公開Homeの主操作は記録閲覧。紹介は既存の厳しいeligibilityを維持して1件までとし、別セクションで同じ写真を繰り返さない。
- 一覧は媒体の比率を固定し、題名・日付・未確認状態を同時に読む。現在の一覧read modelに本人の題名がない場合は媒体・日付を使い、非公開メモを公開APIへ足して補完しない。取得失敗と0件を分け、利用者が再読込できる。
- 詳細は本人のメモの後にAI候補を置く。状態パネルの並べ替えは2回目以降DOMを変えない。mobileの展開情報を、閉じる操作のない全画面固定表示にしない。
- native撮影は選択・入力を端末へ保存してから下書き保存と表示する。入力はまとめて書き込み、送信開始の同期ロックで二重送信を止める。媒体保存完了までは下書きを消さない。
- 下書きは既存の `latest:user:<userId>` / `latest:guest:<continuationToken>` とownerKeyの方式をnative入口・復旧でも使う。異なる所有者やトークンの下書きを表示・上書き・削除しない。所有者不明の旧 `latest` は推測で復元せず、そのまま保持する。
- 固定の緯度経度を観察場所として初期入力しない。現在地取得は本人の操作時だけ行う。**位置なしのサーバー保存はまだ未実装**で、現行upsertが `missing_location` を返す。位置を得られない場合も写真・メモの端末下書きは残る。§16.6の「位置は任意」を達成したとは扱わず、APIと位置の欠損表現を合わせる変更が必要。
- 地図の専門layerは補助操作へまとめる。正式Place検索の実データ不足、出典・権利・Review理由・外部掲載の未接続項目は画面だけで実装済みにしない。M6参加と本人設定は既存の入口・権限を保つ。

本節のsource更新自体は本番稼働の証拠ではない。公開判定は、同じsourceのstaging検証、productionのWorker版とR2 manifest、公開/本人の主要導線の読戻しを中央release Evidenceで確認する。静的画面案の仮データは配備しない。
