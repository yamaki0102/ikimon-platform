...
3. Legacy lane is tiered by changed files. If no legacy PHP / `upload_package` / legacy runtime
   boundary files changed, preserve runtime data/config and the persistent uploads symlink while
   syncing `/var/www/ikimon.life/repo` to the release SHA and skip the full legacy `deploy.sh`
   permission/PHP-FPM path. If sync-only has to repair the uploads symlink, it forces one legacy
   sync so media metadata is rebuilt from persistent storage. Compare `Repo sync sec` against the
   prior `Legacy deploy over SSH` baseline.
