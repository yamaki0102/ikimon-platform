...
一次ソースは 2026-04-23 Explore agent サマリ。以下 2 点が精査対象:

1. 一部ファイル名 (`bootstrap.php`, `admin.php`, `bio-index.php`, `goals.php`, `analyze_audio.php`
   等) は api/ 直下に未実在。サブディレクトリ `api/v2/`, `api/admin/`, `api/business/`,
   `api/affiliate/` にある可能性。**T-24h チェック前に `find` 再実行で実在確認必須**
2. 各 family の count 数値は概算。正確数は `find upload_package/public_html/api -name '*.php'`
   + ファイル名パターン機械分類で確定させる（カットオーバー前の追加タスク）

この分類ドキュメントは **方針と分類ルールの正本** として機能する。各 endpoint 実在確認は
T-24h 最終チェックの Section C cutover_strategy 決定時に突合する。
...
| `admin.php` | misc/utility | block_at_edge | ⚠ 実在未確認。api/admin/ サブ経由の誤認可能性 |
...
| `bio-index.php` | region-stats | proxy_to_legacy | ⚠ 実在未確認。T-24h 再確認 |
| `bootstrap.php` | misc/utility | proxy_to_legacy | ⚠ **api/ 直下に実在しない**。Explore サマリ誤認。削除対象（要確認後 dev-debug 移送 or ドロップ） |
...
- **要精査**: `bootstrap.php` — ⚠ api/ 直下に実在しない疑い。`find` 再実行で確定後、
  存在すれば dev-only チェック、存在しなければ分類表から削除

## 次のアクション

1. ✅ `docs/strategy/replacement_final_checklist_2026-04-23.md` Section C 更新済
2. **T-24h 精査必須**: `find upload_package/public_html/api -name '*.php' | sort` で全 188 件
   の正確ファイル名を取得、本ドキュメントの疑義マーク (⚠) を突合
3. `bootstrap.php` が実在しなければ分類表から削除、`admin.php` / `bio-index.php` なども
   同様に確認
4. cutover 直前に edge (nginx) で以下のパスを 404/403 化:
   - `/api/dev_*`, `/api/*debug*`, `/api/verify_*` (既存 dev-debug family: csrf_debug.php,
     test_concurrency.php, verify_config.php は直下に実在確認済)
