...
| 2026-04-23 22:55 | 愛 (Claude) | **Path 誤認 判明 + 再々 bootstrap**。ユーザー指摘「ステージングトップに本番で出ているデータがない」→ 調査で Nats `user_69be85c688371`（4月観察、orphan user）が staging に存在しないと判明。真の data path は `/var/www/ikimon.life/repo/upload_package/data`（**path B**）であり、21:37 の bootstrap で使っていた `/var/www/ikimon.life/data`（**path A**）は古いスナップショット。加えて `import:legacy` 以外の 6 scripts が `LEGACY_DATA_ROOT` env を読まずハードコードで staging repo mirror を参照するバグ。対応: staging + 本番 v2 DB を再 TRUNCATE → `--legacy-data-root=path_B` CLI arg で bootstrap 実行。結果: **users 77 / observations 6530 / visits 6573 / trackPoints 5725**（27 倍差）。本番 v2 /ops/readiness 全 gates GREEN、Nats の 2026-04-23 浜松 / 2026-04-19 京都×4 の最新観察が DB 入り、staging `/` で表示確認。 |
