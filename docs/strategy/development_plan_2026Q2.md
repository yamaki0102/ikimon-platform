...
AIの役割は「候補の提案」まで。最終同定は必ず人間（ユーザーの観察投稿 + 同定コミュニティ）が行う。

```
AI候補提案 (Tier 1)
  → ユーザーが投稿で確認 (Tier 1.5)
    → コミュニティ同定 (Tier 2)
      → 合意形成 = Research Grade (Tier 3)
```

TFLite / Perch v2 / Nano は全て「提案エンジン」。
データの信頼性を担保するのは、人間の目と知識。
これは ikimon.life の Evidence Tier 設計と完全に一致する。
