...
2026-04-23 17:10 JST: migration 0017-0020 を本番 DB に適用、Known Limitation 全解消。
残るのは production v2 dist の rebuild + pm2 restart のみ (cutover 後 T+30m)。

適用済 migration:
- 0017_taxa_gbif_cache
- 0018_ai_runs_and_visit_display_state
- 0018_specialist_authorities
- 0019_authority_recommendations
- 0020_audio_privacy_and_bundles

### GO 条件達成（誤判定）→ rollback（2026-04-23 21:14 JST）

本番 v2 全 gates GREEN、Section E.1 の hard_stop チェックは全て PASS / WAIVED **と判断したが、
cutover 実行後 15 分で S2 incident、rollback した**。

**根本原因**: 本番 v2 DB (`ikimon_v2`) に legacy データの bootstrap import が未完遂
（`counts.users=9` vs legacy ~100、`trackPoints=0`、最新 bootstrap_import.rows_imported=0）。
gates.parityVerified の意味が「staging DB ↔ legacy mirror の同期健全性」で、
「本番 v2 DB の完成度」を保証するものではなかった。

**再発防止**:
- Section D に **Data Freshness gate** を追加済（`counts.users >= legacy × 0.95`、`trackPoints > 0`、`bootstrap_import.rows_imported > 0`）
- Section E.1 に F0-F2 新規チェック追加済、全 FAIL で re-cutover を blocker
- CUTOVER_RUNBOOK のトリガー条件に counts 乖離を追加済

**再カットオーバー前の必須前処理**: [`ops/runbooks/cutover_retry_bootstrap_2026-04-23.md`](../../ops/runbooks/cutover_retry_bootstrap_2026-04-23.md) 参照
**Incident report**: [`ops/incidents/INC_2026-04-23_v2_cutover_rollback.md`](../../ops/incidents/INC_2026-04-23_v2_cutover_rollback.md)
...
### 🔴 再カットオーバー前の必須前処理（2026-04-23 INC 発生）

- [ ] 本番 v2 DB (`ikimon_v2`) への bootstrap import 完遂
  - `npm run import:legacy` → `import:observations` → `:evidence` → `:identifications` → `:conditions` → `import:remember-tokens` → `import:tracks`
- [ ] `counts.users >= legacy users.json × 0.95` を確認
- [ ] `counts.trackPoints > 0` を確認
- [ ] 最新 `bootstrap_import.rows_imported > 0` を確認
- [ ] checklist Section E.1 F0-F2 を PASS にする
- [ ] その後 nginx 設定を `/etc/nginx/sites-available/ikimon.life.v2-cutover-snapshot` から復元
  （`cp snapshot → ikimon.life && nginx -t && reload`）

手順詳細: [`ops/runbooks/cutover_retry_bootstrap_2026-04-23.md`](../../ops/runbooks/cutover_retry_bootstrap_2026-04-23.md)
