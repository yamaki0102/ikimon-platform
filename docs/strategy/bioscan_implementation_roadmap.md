# BioScan + ikimon.life 100年アーカイブ戦略 — 実装計画

## Context

「2026年に記録しなかったデータは二度と取り戻せない」

これが ikimon.life の哲学。BioScan は「最強のアプリ」ではなく「最も多くの環境タイムカプセルを生成する装置」。ikimon.life はそれを受け止め、100年残し、社会に返す基盤。

Codex レビュー + 戦略文書 (`bioscan_ikimon_100y_archive_strategy_2026-03-24.md`) を踏まえ、**既存実装とのギャップ**を埋める具体的改修計画を立てる。
...
### Phase 1: BioScan 基盤修正（データを流す）

**目標**: タイムカプセルの価値の根幹を作り、ikimon.life にデータを流す

#### 1-1. TFLite 種同定モデル導入
- **ファイル**: `ikimon-bioscan/app/src/main/java/life/ikimon/bioscan/ai/TFLiteEngine.kt`（新規）
...
- **ファイル**: `ikimon-bioscan/app/src/main/java/life/ikimon/bioscan/data/SyncManager.kt`
- **内容**: `SYNC_ENABLED = true` に切替、`passive_event.php` への送信テスト
- **既存対応**: `passive_event.php` は `client_session_id` マージ対応済み → BioScan のセッションIDを渡すだけ

#### 1-4. 加速度計スキャン間隔制御
- **ファイル**: `ikimon-bioscan/app/src/main/java/life/ikimon/bioscan/scanner/BioScanEngine.kt`
...
#### 2-2. Pixel 10 Pro センサー統合（BioScan側）
- **ファイル**: `ikimon-bioscan/app/src/main/java/life/ikimon/bioscan/scanner/SensorScanner.kt`（新規）
- **内容**: `SensorManager` から温度計・気圧計・照度・磁力計を60秒毎に読み取り
- **送信**: `env_history` として `passive_event.php` に送信

#### 2-3. 広角コンテキスト写真の自動撮影
- **ファイル**: `ikimon-bioscan/app/src/main/java/life/ikimon/bioscan/scanner/VisualScanner.kt`
...
#### 2-4. 音響指数計算
- **ファイル**: `ikimon-bioscan/app/src/main/java/life/ikimon/bioscan/scanner/AcousticAnalyzer.kt`（新規）
...
- **拡張**: BioScan セッションごとのタイムカプセル詳細ページ
...
1. `docs/strategy/bioscan_100y_archive_strategy.md` — Codex 戦略文書のコピー
2. `docs/strategy/bioscan_codex_review.md` — 今日のCodexレビュー（10項目の衝突と結論）
3. `docs/strategy/bioscan_implementation_roadmap.md` — この実装ロードマップ

### BIOSCAN_V02_PLAN.md の更新

`ikimon-bioscan/docs/BIOSCAN_V02_PLAN.md` を 100年アーカイブ哲学ベースに全面改訂
...
| `docs/BIOSCAN_V02_PLAN.md` | 全面改訂 | 今日 |
...
| ファイル | 操作 |
|---------|------|
| `ikimon.life/docs/strategy/bioscan_100y_archive_strategy.md` | 新規（戦略文書保存） |
| `ikimon.life/docs/strategy/bioscan_codex_review.md` | 新規（レビュー保存） |
| `ikimon.life/docs/strategy/bioscan_implementation_roadmap.md` | 新規（ロードマップ） |
| `ikimon.life/CLAUDE.md` | 改修（哲学+Phase追加） |
| `ikimon-bioscan/docs/BIOSCAN_V02_PLAN.md` | 全面改訂 |
