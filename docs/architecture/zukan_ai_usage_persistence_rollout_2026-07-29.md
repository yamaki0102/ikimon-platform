...
- PostgreSQL `0140_ai_usage_control.sql`
...
- PostgreSQL `0141_ai_usage_control_hardening.sql`
...
1. Apply and validate `0140`–`0141` on a disposable PostgreSQL database.
...
5. Compare call count, USD totals, provider/model distribution, cache evidence, and event lineage.
6. Switch budget reads to gross usage from `ai_usage_events`.
7. Stop authoritative writes to `ai_cost_log` after the approved compatibility window.
...
Cloudflare Worker or D1 AI execution must not use the in-memory repository. Before Worker routing is activated, add a D1/Durable Object implementation with equivalent atomic lease, append-only event, retry-lineage, and USD-budget semantics.

## Validation before migration approval

Required:

- canonical `npm --prefix platform_v2 run typecheck`
- canonical `npm --prefix platform_v2 run test:node`
- fresh PostgreSQL application of `0140`–`0141`
...
- retry and adjustment cross-tenant rejection test
- raw usage content rejection test
