            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-3xl shadow-sm p-6 border border-slate-100">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- Demographics (Shown only if Event Code is entered) -->
                        <div x-show="eventTag.trim().length > 0" x-cloak class="space-y-4 p-4 bg-teal-50 rounded-xl border border-teal-100 mb-4 mt-4">
                            <p class="text-xs text-teal-700 font-bold mb-2">イベント参加の場合は以下の属性をご回答ください（助成金等への調査報告に利用されます）</p>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">年齢層 <span class="text-red-500">*</span></label>
                                <select x-model="ageGroup" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-teal-500">
                                    ...
                                    <option value="60s_plus">60代以上</option>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">参加属性 <span class="text-red-500">*</span></label>
                                <select x-model="participantType" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-teal-500">
                                    <option value="">選択してください</option>
                                    <option value="local">地域住民</option>
                                    <option value="visitor">観光・市外からの来訪者</option>
                                    <option value="expert">専門家・研究者</option>
    ...
    <script nonce="<?= CspNonce::attr() ?>">
        function surveyApp() {
            return {
                ...
                eventTag: '<?php echo htmlspecialchars($_GET["event_code"] ?? "", ENT_QUOTES); ?>',
                ageGroup: '',
                participantType: '',
                partnerName: '',
                party: [],
                ...
                async startSurvey() {
                    const evtTagVal = this.eventTag.trim();
                    if (evtTagVal.length > 0) {
                        if (!this.ageGroup || !this.participantType) {
                            alert('イベントに参加する場合は「年齢層」と「参加属性」を選択してください。');
                            return;
                        }
                    }
                    ...
                    if (evtTagVal) {
                        formData.append('event_tag', evtTagVal);
                        formData.append('age_group', this.ageGroup);
                        formData.append('participant_type', this.participantType);
                    }
                    if (this.weatherType) {
