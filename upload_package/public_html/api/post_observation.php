...
if (DataStore::append('observations', $observation)) {
    ...
    if ($aiPlan !== null) {
        try {
            ...
            error_log("enqueue AI failed for {$id}: " . $e->getMessage());
            DataStore::upsert('observations', ['id' => $id, 'ai_assessment_status' => 'enqueue_failed']);
            $aiPlan = null;
        }
    }
    if ($embeddingPlanned) {
        try {
            ...
            error_log("enqueue embedding failed for {$id}: " . $e->getMessage());
            $embeddingPlanned = false;
        }
    }
