...
// Tier Gate: DwC-A export requires 'researcher' tier or above
...
function buildDwcRow(array $obs, string $domain, array $licenseMap): array
{
    ...
    // Phase D M8: 100-Year Archive - Historical Taxonomy Mapping
    // `dwc:originalNameUsage` captured verbatim from literature, `dwc:locality` for exact region phrases, `dwc:nameAccordingTo` for the source classification
    $originalNameUsage = $obs['originalNameUsage'] ?? '';
    // If not on observation level, see if taxon has it via literature ingestion
    if (empty($originalNameUsage)) {
        // As a fallback for literature-sourced data without direct originalNameUsage on obs
...
if ($format === 'archive') {
    ...
    // Create ZIP
    $zipFile = sys_get_temp_dir() . '/ikimon_dwca_' . date('Ymd_His') . '.zip';
    $zip = new ZipArchive();
    ...
    header('Content-Disposition: attachment; filename="ikimon_dwca_' . date('Ymd_His') . '.zip"');
...
header('Content-Disposition: attachment; filename="ikimon_dwca_export_' . date('Ymd_His') . '.csv"');
header('Cache-Control: no-cache, no-store, must-revalidate');
