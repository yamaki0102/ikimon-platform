...
<div x-data="npsSurvey" x-show="showModal" x-cloak
    ...
    <!-- Modal -->
    <div class="relative glass-card rounded-3xl p-8 max-w-md w-full border border-gray-200 shadow-2xl"
        x-transition:enter="transition ease-out duration-300 delay-100"
        x-transition:enter-start="opacity-0 scale-95"
        x-transition:enter-end="opacity-100 scale-100">

        <button @click="dismiss()" class="absolute top-4 right-4 text-gray-400 hover:text-gray-900">
            ...
            <div class="flex justify-center gap-1 mb-4">
                <template x-for="n in 11" :key="n-1">
                    <button @click="score = n-1"
                        :class="score === (n-1) ? 'bg-[var(--color-primary)] text-black' : 'bg-gray-100 hover:bg-gray-200'"
