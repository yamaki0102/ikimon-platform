...
<body class="js-loading pt-14 bg-base text-text font-body">
    ...
    <main class="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-32 pb-32">
            ...
            <div class="flex-1 text-center md:text-left">
                ...
                <div class="flex flex-wrap justify-center md:justify-start gap-8 mt-8">
                    <div class="text-center">
                        <p class="text-4xl md:text-5xl font-heading font-black text-primary tracking-tight"><?php echo count($user_obs); ?></p>
                        <p class="font-bold text-muted uppercase tracking-widest mt-1" style="font-size: var(--text-xs);">記録</p>
                    </div>
                    <div class="text-center">
                        <p class="text-4xl md:text-5xl font-heading font-black text-text tracking-tight"><?php echo count($life_list); ?></p>
        ...
        <section class="mb-12" x-data="ambientProfile()" x-init="loadData()">
            ...
            <!-- デイリークエスト -->
            <div class="bg-surface border border-border rounded-2xl p-5 mb-4" x-data="dailyQuests()" x-init="load()" x-show="quests.length > 0" x-cloak>
                <h3 class="text-sm font-black text-text mb-3 flex items-center gap-2">
                    <i data-lucide="target" class="w-4 h-4 text-warning"></i>
                    今日のクエスト
                </h3>
                <div class="space-y-2">
                    <template x-for="q in quests" :key="q.id">
                        <div class="flex items-center gap-3 p-2.5 rounded-xl" :class="q.completed ? 'bg-primary-surface' : 'bg-[var(--color-bg-base)]'">
                            <span class="text-lg shrink-0" x-text="q.icon"></span>
                            <div class="flex-1 min-w-0">
                                <p class="text-xs font-bold" :class="q.completed ? 'text-primary line-through' : 'text-text'" x-text="q.title"></p>
                                <div class="mt-1 h-1.5 rounded-full bg-border overflow-hidden">
                                    <div class="h-full rounded-full transition-all duration-500"
                                         :class="q.completed ? 'bg-primary' : 'bg-warning'"
                                         :style="'width: ' + q.progress + '%'"></div>
                                </div>
                            </div>
                            <span class="text-[10px] font-black shrink-0" :class="q.completed ? 'text-primary' : 'text-warning'" x-text="q.completed ? '✓' : q.reward + 'pt'"></span>
                        </div>
                    </template>
                </div>
            </div>
            ...
            <!-- 友達を招待 QRコード -->
            <?php
            $inviteCode = InviteManager::getOrCreateCode($user['id']);
            $inviteUrl = InviteManager::getInviteUrl($inviteCode);
            $inviteStats = InviteManager::getStats($user['id']);
            ?>
            <div class="bg-surface border border-border rounded-2xl p-5 mb-4" x-data="inviteCard()" x-init="init()">
                <h3 class="text-sm font-black text-text mb-3 flex items-center gap-2">
                ...
                <div class="flex items-start gap-4">
                    <!-- QRコード -->
                    <div class="shrink-0 cursor-pointer" @click="$dispatch('open-qr-fullscreen'); if(window.ikimonAnalytics) ikimonAnalytics.track('qr_fullscreen', {})">
                        <div id="qrcode-mini" class="w-28 h-28 bg-white rounded-xl p-1.5 border border-border shadow-sm hover:shadow-md hover:border-primary/30 transition"></div>
                        <p class="text-[9px] text-faint text-center mt-1">タップで拡大</p>
                    </div>
                    <!-- 説明+ボタン -->
                    <div class="flex-1 min-w-0">
                        <p class="text-xs text-muted leading-relaxed mb-3">
                            このQRコードを見せるだけで、友達を ikimon.life に招待できるよ
                        </p>
                        <div class="flex gap-2 flex-wrap">
                            <button @click="copyLink()" class="px-3 py-1.5 rounded-full bg-primary-surface border border-primary/20 text-primary text-xs font-bold hover:bg-primary/10 active:scale-95 transition flex items-center gap-1">
                                <i data-lucide="link" class="w-3 h-3"></i> リンクをコピー
                            </button>
                            <button @click="saveQR()" class="px-3 py-1.5 rounded-full bg-surface border border-border text-muted text-xs font-bold hover:text-text active:scale-95 transition flex items-center gap-1">
                                <i data-lucide="download" class="w-3 h-3"></i> QRを保存
                            </button>
                        </div>
                        <!-- コピー成功トースト -->
                        <p x-show="copied" x-transition class="text-xs text-primary font-bold mt-2">✓ コピーしました！</p>
                        <?php if ($inviteStats['accept_count'] > 0): ?>
                        <div class="mt-3 flex items-center gap-1.5">
                            <p class="text-[10px] text-muted font-bold">招待した人:</p>
                            <span class="text-xs font-black text-primary"><?php echo $inviteStats['accept_count']; ?>人</span>
                            <div class="flex -space-x-1.5 ml-1">
                                <?php foreach (array_slice(array_reverse($inviteStats['accepted_users']), 0, 5) as $au): ?>
                                    <div class="w-5 h-5 rounded-full bg-surface border border-border flex items-center justify-center text-[8px] font-bold text-muted" title="<?php echo htmlspecialchars($au['name'] ?? ''); ?>">
                                        <?php echo mb_substr($au['name'] ?? '?', 0, 1); ?>
                                    </div>
                                <?php endforeach; ?>
                            </div>
                        </div>
            ...
            <div x-data="{ showFullscreen: false }"
                 @open-qr-fullscreen.window="showFullscreen = true"
                 ...
                 x-cloak>
                <!-- 閉じるボタン -->
                <button @click="showFullscreen = false" class="absolute top-4 right-4 w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200 transition">
                    <i data-lucide="x" class="w-5 h-5 text-gray-600"></i>
                </button>

                <!-- QRコード（大） -->
                <div id="qrcode-fullscreen" class="mb-6"></div>

                <!-- ブランド -->
                <p class="text-lg font-black text-gray-900 tracking-tight">ikimon.life</p>
                <p class="text-sm text-gray-500 font-medium mt-1"><?php echo htmlspecialchars($user['name']); ?></p>
                <p class="text-xs text-gray-400 mt-3 max-w-[240px] text-center leading-relaxed">自然を見る目が育つプラットフォーム</p>
            </div>

            <!-- 成長ログ (Growth Tracker) -->
            <?php
            $growthSnap = GrowthTracker::updateSnapshot($user['id']);
            $growthMessages = GrowthTracker::generateMessages($user['id']);
            $growthHistory = GrowthTracker::getHistory($user['id']);
            // 初回/最新観察を取得
            $userAllObs = DataStore::fetchAll('observations');
            $userObs = array_values(array_filter($userAllObs, function ($o) use ($user) {
                return ($o['user_id'] ?? '') === $user['id'];
            }));
            usort($userObs, function ($a, $b) {
                return strcmp($a['created_at'] ?? '', $b['created_at'] ?? '');
            });
            $firstObs = !empty($userObs) ? $userObs[0] : null;
            $latestObs = count($userObs) > 1 ? $userObs[count($userObs) - 1] : null;
            $gm = $growthSnap['metrics'] ?? [];
            ?>
            <?php if (($gm['total_observations'] ?? 0) > 0): ?>
            <div class="bg-surface border border-border rounded-2xl p-5 mb-4">
                ...
                <!-- 成長メッセージ -->
                <?php if (!empty($growthMessages)): ?>
                <div class="space-y-1.5 mb-4">
                    <?php foreach ($growthMessages as $msg): ?>
                    <p class="text-xs text-primary font-bold flex items-start gap-1.5">
                ...
                <div class="grid grid-cols-3 gap-2 mb-4">
                    <div class="text-center p-3 rounded-xl bg-[var(--color-bg-base)]">
                        <p class="text-lg font-black text-primary"><?php echo number_format($gm['avg_taxon_depth'] ?? 0, 1); ?></p>
                        <p class="text-[10px] font-bold text-muted">分類精度</p>
                    </div>
                    <div class="text-center p-3 rounded-xl bg-[var(--color-bg-base)]">
                        <p class="text-lg font-black text-secondary"><?php echo (int)($gm['evidence_diversity'] ?? 0); ?></p>
                        <p class="text-[10px] font-bold text-muted">観察ポイント</p>
                    </div>
                    <div class="text-center p-3 rounded-xl bg-[var(--color-bg-base)]">
                        <p class="text-lg font-black text-accent"><?php echo (int)($gm['taxon_breadth'] ?? 0); ?></p>
                        <p class="text-[10px] font-bold text-muted">分類群</p>
                    </div>
                </div>

                <!-- 初回 vs 最新の観察比較 -->
                <?php if ($firstObs && $latestObs && $firstObs['id'] !== $latestObs['id']): ?>
                <div class="border-t border-border pt-4">
                    <p class="text-[10px] font-black text-muted uppercase tracking-widest mb-3">はじめての観察 → 最新の観察</p>
                    <div class="grid grid-cols-2 gap-3">
                        <!-- 初回 -->
                        <a href="observation_detail.php?id=<?php echo htmlspecialchars($firstObs['id']); ?>" class="block">
                            <div class="rounded-xl overflow-hidden border border-border">
                                <img src="<?php echo htmlspecialchars($firstObs['photos'][0]); ?>" alt="" class="w-full h-20 object-cover" loading="lazy">
                                <div class="p-2">
                                    <p class="text-[10px] text-faint"><?php echo date('Y/m/d', strtotime($firstObs['created_at'] ?? '')); ?></p>
                                    <p class="text-xs font-bold text-text truncate"><?php echo htmlspecialchars($firstObs['taxon']['name'] ?? '未同定'); ?></p>
                                </div>
                            </div>
                        </a>
                        <!-- 最新 -->
                        <a href="observation_detail.php?id=<?php echo htmlspecialchars($latestObs['id']); ?>" class="block">
                            <div class="rounded-xl overflow-hidden border border-primary/30">
                                <img src="<?php echo htmlspecialchars($latestObs['photos'][0]); ?>" alt="" class="w-full h-20 object-cover" loading="lazy">
                                <div class="p-2">
                                    <p class="text-[10px] text-faint"><?php echo date('Y/m/d', strtotime($latestObs['created_at'] ?? '')); ?></p>
                                    <p class="text-xs font-bold text-primary truncate"><?php echo htmlspecialchars($latestObs['taxon']['name'] ?? '未同定'); ?></p>
                                </div>
                            </div>
                        </a>
                    </div>
            ...
            <div class="grid grid-cols-2 gap-3 mt-2">
                <a href="#" @click.prevent="showReport = true" class="bg-gradient-to-r from-primary-surface to-secondary-surface border border-primary-glow rounded-2xl p-4 hover:border-primary/40 transition group">
                    <h3 class="text-sm font-black text-text group-hover:text-primary transition">📊 足あとレポート</h3>
                    <p class="text-xs text-muted">今年の活動を振り返る</p>
                </a>
                <a href="wellness.php" class="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-4 hover:border-green-400 transition group">
                    <h3 class="text-sm font-black text-gray-800 group-hover:text-green-700 transition">🌿 ウェルネス</h3>
                    <p class="text-xs text-gray-500">自然活動の記録を見る</p>
                </a>
            </div>
        </section>
        ...
        ?>
        <div x-data="{ tab: new URLSearchParams(location.search).get('tab') || 'observations' }">
            <div class="sticky top-20 z-30 bg-base/95 backdrop-blur-md border-b border-border mb-12 flex gap-4 md:gap-8 pt-4 overflow-x-auto scrollbar-hide">
                ...
                <button @click="tab = 'discoveries'" class="pb-4 text-sm font-bold tracking-widest uppercase transition border-b-2 whitespace-nowrap" :class="tab === 'discoveries' ? 'text-primary border-primary' : 'text-muted border-transparent hover:text-text'">🌿 発見</button>
                <button @click="tab = 'wellness'" class="pb-4 text-sm font-bold tracking-widest uppercase transition border-b-2 whitespace-nowrap" :class="tab === 'wellness' ? 'text-primary border-primary' : 'text-muted border-transparent hover:text-text'">💚 ウェルネス</button>
            ...
            <div x-show="tab === 'observations'" x-transition>
                ...
                <div class="bg-gradient-to-r from-blue-900 to-purple-900 rounded-2xl p-4 mb-6 text-white">
                    <div class="flex items-center justify-between">
                        ...
                        <a href="field_research.php" class="text-[10px] text-blue-300 hover:text-white transition">いきものサーチ →</a>
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                    ...
                    <?php else: ?>
                        <?php foreach (array_reverse($user_obs) as $obs):
                            $hasPhoto = !empty($obs['photos'][0]);
                            $obsSrc = $obs['observation_source'] ?? $obs['source'] ?? 'post';
                        ?>
                            <a href="observation_detail.php?id=<?php echo $obs['id']; ?>" class="group block">
                                <div class="aspect-square rounded-2xl overflow-hidden mb-3 relative">
                                    <?php if ($hasPhoto): ?>
                                    <img src="<?php echo htmlspecialchars($obs['photos'][0]); ?>" alt="<?php echo htmlspecialchars($obs['taxon']['name'] ?? '観察写真'); ?>" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                                    <div class="absolute inset-0 bg-black/20 group-hover:bg-transparent transition"></div>
                                    <?php elseif ($obsSrc === 'live-scan-summary'): ?>
                                    <div class="w-full h-full bg-gradient-to-br from-blue-900 to-purple-900 flex flex-col items-center justify-center gap-2">
                                        <span class="text-3xl">📡</span>
                                        <span class="text-xs text-white/70">ライブスキャン</span>
                                    </div>
                                    <?php elseif ($obsSrc === 'walk'): ?>
                                    <div class="w-full h-full bg-gradient-to-br from-emerald-900 to-teal-900 flex flex-col items-center justify-center gap-2">
                                        <span class="text-3xl">🐦</span>
                                        <span class="text-xs text-white/70">ウォーク検出</span>
                                    </div>
                                    <?php else: ?>
                                ...
                                <p class="text-muted mt-1" style="font-size: var(--text-xs);"><?php echo date('Y.m.d', strtotime($obs['observed_at'] ?? $obs['created_at'] ?? 'now')); ?></p>
                            </a>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
            ...
            <!-- Discoveries Tab (デジタル標本箱) -->
            <div x-show="tab === 'discoveries'" x-transition>
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <h3 class="text-xl font-black text-text">デジタル標本箱</h3>
                        <p class="text-sm text-muted font-bold"><?php echo count($life_list); ?>種の発見</p>
                    </div>
                </div>
                <?php if (empty($life_list)): ?>
                    <div class="py-20 text-center">
                        ...
                        <a href="post.php" class="btn-primary"><i data-lucide="camera" class="w-5 h-5"></i> 投稿する</a>
                    </div>
                <?php else: ?>
                            ...
                            <a href="observation_detail.php?id=<?php echo $species['id']; ?>" class="group relative block aspect-[4/5] rounded-2xl overflow-hidden bg-surface border-2 transition hover:-translate-y-1 hover:shadow-xl <?php echo $glowClass; ?>" style="border-color: <?php echo $isRare ? $rarityColor : 'var(--color-border)'; ?>">
                                <div class="absolute inset-0">
                                    <?php if ($species['photo']): ?>
                                        <img src="<?php echo htmlspecialchars($species['photo']); ?>" alt="<?php echo htmlspecialchars($species['name'] ?? '観察写真'); ?>" class="w-full h-full object-cover transition duration-700 group-hover:scale-110" loading="lazy">
                                    <?php else: ?>
                                        <div class="w-full h-full flex items-center justify-center bg-base text-muted"><i data-lucide="image" class="w-8 h-8 opacity-50"></i></div>
                                    <?php endif; ?>
                                ...
                                <div class="absolute bottom-4 left-4 right-4 text-white">
                                    <h4 class="font-bold text-lg leading-tight mb-1 truncate drop-shadow-md"><?php echo htmlspecialchars($species['name']); ?></h4>
            ...
            <!-- Wellness Tab -->
            <div x-show="tab === 'wellness'" x-transition>
                <div class="space-y-8">
                    <!-- Weekly Nature Time -->
                    <div class="glass-card p-6 md:p-8 rounded-3xl border-border">
                        <div class="flex items-center justify-between mb-6">
                            <div class="flex items-center gap-3">
                                <span class="text-2xl">🌿</span>
                                <div>
                                    <h3 class="text-lg font-black text-text">ネイチャーウェルネス</h3>
                                    <p class="text-xs text-muted font-bold">自然がもたらす心身の健康</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-8">
                            ...
                            <div class="relative h-4 rounded-full bg-[var(--color-bg-base)] overflow-hidden">
                            ...
                            <p class="text-muted mt-2 text-xs" x-show="wellnessCurrentWeekMin >= 120">&#127881; WHO推奨の週120分を達成！</p>
                        </div>

                        <!-- 4 Stat Cards -->
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-base)]">
                                <p class="text-2xl font-black text-primary" x-text="wellnessCurrentWeekMin + '分'"></p>
                                <p class="font-bold text-muted uppercase tracking-wider mt-1" style="font-size: var(--text-xs);">&#127795; 自然時間</p>
                            </div>
                            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-base)]">
                                <p class="text-2xl font-black text-secondary" x-text="(wellness?.physical?.session_count ?? 0) + '回'"></p>
                                <p class="font-bold text-muted uppercase tracking-wider mt-1" style="font-size: var(--text-xs);">&#129406; セッション</p>
                            </div>
                            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-base)]">
                                <p class="text-2xl font-black text-accent" x-text="wellness?.cognitive?.cognitive_engagement ?? 0"></p>
                                <p class="font-bold text-muted uppercase tracking-wider mt-1" style="font-size: var(--text-xs);">&#129504; 認知エンゲージメント</p>
                                <p class="text-muted mt-0.5" style="font-size: 9px;">参考指標</p>
                            </div>
                            <div class="text-center p-4 rounded-xl bg-[var(--color-bg-base)]">
                                <p class="text-2xl font-black text-text" x-text="(wellness?.emotional?.lifelist_total ?? 0) + '種'"></p>
                                <p class="font-bold text-muted uppercase tracking-wider mt-1" style="font-size: var(--text-xs);">&#128203; ライフリスト</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="glass-card p-6 rounded-2xl border-border">
                            <h4 class="text-sm font-black text-text mb-4 flex items-center gap-2">&#129504; 認知ウェルネス</h4>
                            <div class="space-y-3">
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-muted font-bold">観察密度</span>
                                    <span class="text-sm font-black text-text" x-text="(wellness?.cognitive?.observation_density ?? 0).toFixed(1) + ' / 日'"></span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-muted font-bold">種多様性</span>
                                    <span class="text-sm font-black text-text" x-text="(wellness?.cognitive?.species_diversity ?? 0) + ' 種'"></span>
                                </div>
                            </div>
                        </div>
                        <div class="glass-card p-6 rounded-2xl border-border">
                            <h4 class="text-sm font-black text-text mb-4 flex items-center gap-2">&#128154; 感情ウェルネス</h4>
                            <div class="space-y-3">
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-muted font-bold">連続記録</span>
                                    <span class="text-sm font-black text-text" x-text="(wellness?.emotional?.streak_days ?? 0) + ' 日'"></span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-muted font-bold">新種発見</span>
                                    <span class="text-sm font-black text-text" x-text="(wellness?.emotional?.new_species ?? 0) + ' 種'"></span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-xs text-muted font-bold">ライフリスト</span>
                                    <span class="text-sm font-black text-text" x-text="(wellness?.emotional?.lifelist_total ?? 0) + ' 種'"></span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="glass-card p-6 rounded-2xl border-border">
                        ...
                        <div class="flex gap-2 mt-4">
                            <a href="field_research.php" class="flex-1 block text-center text-sm font-bold py-2.5 rounded-xl" style="background:rgba(16,185,129,0.1);color:#10b981;">
                                🔍 いきものサーチを始める
                            </a>
                            <a href="century_archive.php" class="flex-1 block text-center text-xs font-bold py-2.5 rounded-xl" style="background:rgba(16,185,129,0.05);color:#10b981;">
                                100年アーカイブ →
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <a href="wellness.php" class="glass-card p-5 rounded-2xl border-border flex items-center gap-4 hover:border-primary/40 transition group">
                            ...
                            <div class="flex-1">
                                <h4 class="text-sm font-black text-text group-hover:text-primary transition">ウェルネス</h4>
                                <p class="text-xs text-muted">自然活動の記録を見る</p>
                            </div>
                            <i data-lucide="chevron-right" class="w-5 h-5 text-faint group-hover:text-primary transition"></i>
                        </a>
                        <a href="wellness.php" class="glass-card p-5 rounded-2xl border-border flex items-center gap-4 hover:border-primary/40 transition group">
                            ...
                            <div class="flex-1">
                                <h4 class="text-sm font-black text-text group-hover:text-primary transition">詳細レポート</h4>
                                <p class="text-xs text-muted">週間・月間のウェルネスデータ</p>
                            </div>
                            <i data-lucide="chevron-right" class="w-5 h-5 text-faint group-hover:text-primary transition"></i>
                        </a>
                    </div>
    ...
    <script nonce="<?= CspNonce::attr() ?>">
        ...
        function dailyQuests() {
            return {
                ...
                async load() {
                    try {
                        const res = await fetch('api/get_daily_quests.php');
                        const data = await res.json();
                        if (data.success && data.quests) {
                            this.quests = data.quests;
                            if (window.ikimonAnalytics) ikimonAnalytics.track('quest_card_view', { count: data.quests.length });
                        }
                    } catch (e) {
        ...
        function inviteCard() {
            return {
                ...
                init() {
                    this.$nextTick(() => {
                        // ミニQR
                        const miniEl = document.getElementById('qrcode-mini');
                        ...
                        // フルスクリーンQR
                        const fullEl = document.getElementById('qrcode-fullscreen');
                        ...
                        if (window.ikimonAnalytics) ikimonAnalytics.track('qr_shown', {});
                    });
                },
                async copyLink() {
                    try {
                        ...
                        if (window.ikimonAnalytics) ikimonAnalytics.track('invite_link_copy', {});
                ...
                saveQR() {
                    const canvas = document.querySelector('#qrcode-mini canvas') || document.querySelector('#qrcode-fullscreen canvas');
                    if (!canvas) return;
                    const link = document.createElement('a');
                    link.download = 'ikimon-invite-qr.png';
                    link.href = canvas.toDataURL('image/png');
                    link.click();
                    if (window.ikimonAnalytics) ikimonAnalytics.track('qr_save', {});
                }
            };
        }
