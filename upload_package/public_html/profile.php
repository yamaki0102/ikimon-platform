...
<body class="js-loading pt-14 bg-base text-text font-body">
    ...
    <main class="max-w-7xl mx-auto px-4 md:px-6 py-12 md:py-32 pb-32">
        ...
        <section class="mb-12" x-data="ambientProfile()" x-init="loadData()">
            ...
            <!-- Regional Contribution Card (地域貢献) -->
            <?php
            $regionalStats = $user['regional_stats'] ?? [];
            $homeRegion = $user['home_region'] ?? '';
            $awayRegionCount = $user['away_region_count'] ?? 0;
            $prefNames = [
            ...
            $totalRegions = count($regionalStats);
            ?>
            <?php if (!empty($regionalStats)): ?>
            <div class="bg-surface border border-border rounded-2xl p-6 mt-2">
                <div class="flex items-center justify-between mb-5">
                    <div class="flex items-center gap-3">
                        <span class="text-2xl">🏘️</span>
                        <div>
                            <h3 class="text-sm font-black text-text">地域貢献マップ</h3>
                            <p class="text-muted" style="font-size: var(--text-xs);">あなたの記録が地域の価値を証明しています</p>
                        </div>
                    </div>
                    <div class="text-right">
                        <span class="text-lg font-black text-primary"><?php echo $totalRegions; ?></span>
                        <p class="text-muted" style="font-size: var(--text-xs);">地域</p>
                    </div>
                </div>

                <?php if ($awayRegionCount > 0): ?>
                <div class="flex items-center gap-2 mb-4 px-3 py-2 bg-blue-50 rounded-xl">
                    <span class="text-sm">🌏</span>
                    <p class="text-xs font-bold text-blue-700">あなたは <?php echo $awayRegionCount; ?> 地域の「関係人口」です</p>
                </div>
                <?php endif; ?>
                ...
                <div class="space-y-2">
                    <?php
                    $displayCount = 0;
                    foreach ($regionalStats as $pref => $speciesCount):
                        if ($displayCount >= 5) break;
                        $prefName = $prefNames[$pref] ?? $pref;
                        $isHome = ($pref === $homeRegion);
                        $barWidth = min(100, ($speciesCount / max(1, max($regionalStats))) * 100);
                        $displayCount++;
                    ?>
                    <div class="flex items-center gap-3">
                        <div class="w-20 text-xs font-bold text-text truncate flex items-center gap-1">
                            <?php if ($isHome): ?><span class="text-xs">🏠</span><?php endif; ?>
                            <?php echo htmlspecialchars($prefName); ?>
                        </div>
                        <div class="flex-1 bg-[var(--color-bg-base)] rounded-full h-2.5 overflow-hidden">
                            <div class="h-full rounded-full transition-all duration-700" style="width: <?php echo $barWidth; ?>%; background: linear-gradient(90deg, <?php echo $isHome ? 'var(--color-primary), var(--color-secondary)' : '#60a5fa, #3b82f6'; ?>);"></div>
                        </div>
                        <span class="text-xs font-black <?php echo $isHome ? 'text-primary' : 'text-text'; ?> w-12 text-right"><?php echo $speciesCount; ?>種</span>
                    </div>
                    <?php endforeach; ?>
                    <?php if ($totalRegions > 5): ?>
                        <p class="text-xs text-muted text-center pt-1">他 <?php echo $totalRegions - 5; ?> 地域</p>
                    <?php endif; ?>
                </div>

                <?php if ($homeRegion && isset($regionalStats[$homeRegion])): ?>
                <div class="mt-4 pt-4 border-t border-border/50">
                    <p class="text-xs text-muted">
                        🏠 <?php echo htmlspecialchars($prefNames[$homeRegion] ?? $homeRegion); ?>（ふるさと）に <span class="font-black text-primary"><?php echo $regionalStats[$homeRegion]; ?>種</span> の記録を残しています
                    </p>
                </div>
                <?php endif; ?>
            </div>
                        ...
                        <?php foreach (array_reverse($user_obs) as $obs): ?>
                            <a href="observation_detail.php?id=<?php echo $obs['id']; ?>" class="group block">
                                <div class="aspect-square rounded-2xl overflow-hidden mb-3 relative">
                                    <img src="<?php echo !empty($obs['photos'][0]) ? $obs['photos'][0] : '/assets/img/no-image.webp'; ?>" alt="<?php echo htmlspecialchars($obs['taxon']['name'] ?? '観察写真'); ?>" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
