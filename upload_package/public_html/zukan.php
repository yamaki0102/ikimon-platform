...
{
    $name = $item['name'] ?? '';
    return 'species.php?name=' . urlencode($name);
}
...
<body class="app-body" x-data="zukanApp()" x-init="init()">
    ...
    <section class="zukan-hero">
        <div class="zukan-hero__inner">
            ...
            <p class="zukan-hero__subtitle">みんなで見つけた地域の生き物カタログ</p>
    ...
    <div class="zukan-grid" id="zukan-grid">
        ...
        <template x-for="(item, idx) in speciesList" :key="item.taxon_key">
            <a :href="'species.php?name=' + encodeURIComponent(item.name)"
