...
8. species レベルを勧める基準: 形態形質が1つ以上明瞭に確認でき、かつ地理・季節・生育環境の証拠も一致する場合。形質が見えているのに「迷うから属」は避けてください。属止まりにするなら why_not_more_specific に「何が見えれば種まで絞れるか」を必ず書いてください。
...
11. next_step は、もう一度同じ場所に見に行くときに最も種同定に効く 1 つの行動を、具体的に 1 文で書いてください。「何を・いつ・どの角度から撮るか」を必ず含めてください（例「6月上旬に同じ株へ再訪し、花を真横から撮って距の長さを記録すると種が確定する」）。「環境メモを残す」「再訪する」だけの抽象表現は禁止です。
...
14. missing_evidence は「もう一度同じ場所に見に行く理由」として読まれる前提で、抽象表現を禁止し、必ずこの種・この場所・この季節に固有の具体例を書いてください。次の 4 観点をなるべく網羅し、最大 4 件にまとめてください：
   - 季節変化: 例「初夏に再訪すれば距が伸びて種同定の決め手になる」「秋には果実が熟して属まで絞れる」
   - 形態の決定的部位: 例「花の側面から距の長さと色を撮る」「葉裏の毛の有無をマクロで撮る」
   - 時間帯/挙動: 例「早朝は花が開く（10 時前に再訪）」「夕暮れに雄花の葯が裂開する」
   - 経時記録の価値: 例「同じ株を月1で撮り続けると、この場所の開花フェノロジーが残る」「開花初日と終日を記録すれば気候変化の指標になる」
   「環境メモを残す」「再訪する」「観察を続ける」だけの抽象表現は厳禁です。
