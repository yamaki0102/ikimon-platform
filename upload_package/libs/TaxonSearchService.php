...
class TaxonSearchService
{
    ...
    private static function searchLocal(string $query, int $limit): array
    {
        ...
        // 学名またはhistorical_namesでも検索（和名で足りなかった場合）
        if (count($results) < $limit) {
            ...
            foreach ($taxa as $slug => $data) {
                ...
                if ($sciName && stripos($sciName, $queryLower) !== false) {
                    $match = true;
                } else if (!empty($data['historical_names'])) {
                    // Check against historical names if scientific name didn't match
                    foreach ($data['historical_names'] as $hName) {
                        if (stripos(strtolower($hName), $queryLower) !== false) {
                            $match = true;
                            break;
                        }
                    }
