...
function isProductionControlPath(pathname) {
  return /^\.github\/workflows\/deploy\.yml$/.test(pathname)
    ...
    || /^scripts\/(?:run_cloudflare_production_(?:release|preflight|materialization|worker_deploy)|verify_cloudflare_production_release|run_production_verification_watch|install_production_verification_service|doctor_production_verification_service)\.sh$/.test(pathname)
    || /^scripts\/(?:Invoke-ProductionVerificationWatch|Install-ProductionVerificationScheduledTask|Test-ProductionVerificationWindows)\.ps1$/.test(pathname)
    || /^scripts\/tests\/(?:production_release_scope\.tests\.mjs|production_verification_evidence\.tests\.mjs|production_verification_operations\.tests\.mjs|release_automation\.tests\.ps1|production_phase_interface\.tests\.sh)$/.test(pathname)
