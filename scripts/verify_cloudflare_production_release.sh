...
for url in \
  ...
  https://zukan.earth/healthz \
  https://zukan.earth/readyz \
  https://zukan.earth/api/v1/runtime/version \
  https://zukan.earth/qa/reflection-loop.json \
  https://zukan.earth/ \
  https://zukan.earth/ja/map \
  https://zukan.earth/ja/learn \
  https://zukan.earth/ja/contact \
...
health_payload="$(fetch_body "https://zukan.earth/healthz?deploy_smoke=${EXPECTED_SHA}")"
require_contains "${health_payload}" "\"buildMarker\":\"${BUILD_MARKER}\"" "Production Worker health payload"

runtime_payload="$(fetch_body "https://zukan.earth/api/v1/runtime/version?deploy_smoke=${EXPECTED_SHA}")"
for marker in \
...
home_body="$(curl -fsS -D "${home_headers}" -H 'cache-control: no-store' "https://zukan.earth/ja/?deploy_release=${EXPECTED_SHA}")"
...
reflection_payload="$(fetch_body "https://zukan.earth/qa/reflection-loop.json?deploy_smoke=${EXPECTED_SHA}")"
...
map_body="$(fetch_body "https://zukan.earth/ja/map?deploy_smoke=${EXPECTED_SHA}")"
signed_map_body="$(curl -fsS -H 'cache-control: no-store' -H 'cookie: ikimon_v2_session=deploy-smoke' "https://zukan.earth/map?deploy_smoke_signed=${EXPECTED_SHA}")"
...
for signed_home_url in \
  "https://zukan.earth/?source=pwa&deploy_smoke_home=${EXPECTED_SHA}" \
  "https://zukan.earth/ja/?source=pwa&deploy_smoke_home=${EXPECTED_SHA}"
do
...
app_sw="$(fetch_body "https://zukan.earth/app-sw.js?deploy_smoke=${EXPECTED_SHA}")"
...
app_refresh="$(curl -fsS -D "${app_refresh_headers}" -H 'cache-control: no-store' -H 'cookie: ikimon_v2_session=deploy-smoke' "https://zukan.earth/app-refresh?to=%2Fmap%3Flang%3Dja%26tab%3Dplaces&deploy_smoke=${EXPECTED_SHA}")"
...
event_body="$(curl -fsS -D "${event_headers}" -H 'cache-control: no-store' "https://zukan.earth/es/community/events/new?field_id=renri-area-sketch-field&deploy_smoke_area_sketch=${EXPECTED_SHA}")"
...
api_status="$(curl -sS -o "${api_body}" -w "%{http_code}" -H 'cache-control: no-store' -H 'content-type: application/json' \
  ...
  "https://zukan.earth/api/v1/fields/renri-area-sketch-field/area-sketch-assessments?deploy_smoke_area_sketch=${EXPECTED_SHA}")"
...
field_body="$(curl -fsS -D "${field_headers}" -H 'cache-control: no-store' "https://zukan.earth/ja/community/fields/1730fc82-95f3-4c64-9c82-4f06d56d940f?deploy_smoke_area_field=${EXPECTED_SHA}")"
...
launcher_body="$(curl -fsS -D "${launcher_headers}" -H 'cache-control: no-store' "https://zukan.earth/?deploy_smoke_record=${EXPECTED_SHA}")"
...
for record_url in \
  "https://zukan.earth/record?deploy_smoke_record=${EXPECTED_SHA}" \
  "https://zukan.earth/ja/record?start=note&deploy_smoke_record=${EXPECTED_SHA}"
do
...
if [[ "${SMOKE_TIER}" == "full" ]]; then
  ...
  OBSERVATION_DETAIL_BASE_URL=https://zukan.earth \
...
const obs = await fetch("https://zukan.earth/ja/api/v1/map/observations?bbox=122.9,24.0,146.0,45.6&zoom=5&limit=20", {
...
for (const ownerPath of ["/api/v1/map/my-observations", "/api/v1/me/map-observations"]) {
  const ownerResponse = await fetch(`https://zukan.earth${ownerPath}`, { headers: { "cache-control": "no-store" } });
...
const areas = await fetch("https://zukan.earth/ja/api/v1/map/area-polygons?bbox=137.55,34.61,137.91,34.85&zoom=12&limit=50", {
  headers: { "cache-control": "no-store" }
}).then((response) => response.json());
...
const parkFixture = await fetch(`https://zukan.earth/ja/api/v1/map/area-polygons?bbox=137.6940,34.6910,137.7075,34.7010&zoom=17&sources=osm_park&limit=120&deploy_smoke=${deploySmoke}`, {
  headers: { "cache-control": "no-store" }
}).then((response) => response.json());
...
const brief = await fetch("https://zukan.earth/ja/api/v1/map/site-brief?lat=34.6942&lng=137.7029&lang=ja", {
  headers: { "cache-control": "no-store" }
}).then((response) => response.json());
