...
HISTORICAL = Path(".tmp-worker-authz/historical")
...
historical_runtime_path = HISTORICAL / "index.ts"
runtime = runtime_path.read_text(encoding="utf-8")
historical_runtime = historical_runtime_path.read_text(encoding="utf-8")
...
runtime = runtime.replace(
    ...
    section(historical_runtime, preferences_start, preferences_end),
    1,
)
...
runtime = runtime.replace(
    ...
    section(historical_runtime, place_memory_start, place_memory_end),
    1,
)
...
test_path = CURRENT / "index.test.ts"
historical_test_path = HISTORICAL / "index.test.ts"
test_source = test_path.read_text(encoding="utf-8")
historical_test = historical_test_path.read_text(encoding="utf-8")
...
test_source = test_source.replace(
    ...
    section(historical_test, place_memory_test_start, place_memory_test_end),
    1,
)
...
if idor_test_start not in test_source:
    idor_test = section(historical_test, idor_test_start, idor_test_end)
