...
if (impactReceiptPath) {
  ...
  const receiptErrors = validateMaterializationImpactReceipt(impactReceipt, {
...
function wranglerR2Get(bucketName, key, filePath) {
  const wranglerCliPath = join(scriptDir, "..", "node_modules", "wrangler", "bin", "wrangler.js");
  const objectPath = `${bucketName}/${normalizeR2ObjectKey(key)}`;
  const commandArgs = [wranglerCliPath, "r2", "object", "get", objectPath, "--file", filePath, "--remote"];
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, commandArgs, {
      cwd: join(scriptDir, ".."), stdio: ["ignore", "pipe", "pipe"], windowsHide: true,
    });
    let stdout = "";
    let stderr = "";
    child.stdout.on("data", (chunk) => { stdout += chunk.toString(); });
    child.stderr.on("data", (chunk) => { stderr += chunk.toString(); });
    child.on("close", (exitCode) => {
      if (exitCode === 0) return resolve({ key, exitCode });
...
function normalizeManifestItem(item, fallbackVersionPrefix) {
  ...
  return {
    ...
    identitySha256: String(item?.identitySha256 ?? item?.identity_sha256 ?? item?.sha256 ?? "").toLowerCase(),
...
async function readPriorMaterializationManifest(bucketName, prior, tempDirPath) {
  ...
  if (!/^original-ui\/versions\/[a-f0-9]{64}$/u.test(versionPrefix)) throw new Error("selective_prior_version_prefix_invalid");
  ...
  const expectedManifestSha256 = String(prior?.manifest_sha256 ?? "").toLowerCase();
  if (expectedManifestSha256 && sha256(payload) !== expectedManifestSha256) throw new Error("selective_prior_manifest_sha256_mismatch");
  let parsed;
  try { parsed = JSON.parse(payload); } catch { throw new Error("selective_prior_manifest_invalid_json"); }
  ...
  if (parsed.bundleHash !== prior.bundle_hash || parsed.versionPrefix !== versionPrefix) throw new Error("selective_prior_manifest_provenance_mismatch");
...
function buildSelectiveManifestItems(allItems, selectedItems, priorManifest, versionPrefix) {
  ...
  for (const item of allItems) {
    ...
    if (!prior || prior.identitySha256 !== item.identitySha256 || prior.bytes !== item.bytes) {
  ...
  for (const item of selectedItems) {
    const key = item.key.replace(/^original-ui\//u, "");
    merged.set(key, {
      key, bytes: item.bytes, sha256: item.sha256, identitySha256: item.identitySha256,
      contentType: item.contentType, version_prefix: versionPrefix,
    });
  }
...
async function directR2Sync(allItems, tempDirPath, bucketName, receipt = null) {
  ...
  const manifestItems = selected.mode === "SELECTIVE_REBUILD"
    ? buildSelectiveManifestItems(allItems, selected.items,
      await readPriorMaterializationManifest(bucketName, receipt.prior_artifact_provenance, tempDirPath), versionPrefix)
    : allItems.map((item) => ({
      key: item.key.replace(/^original-ui\//u, ""), bytes: item.bytes, sha256: item.sha256,
      identitySha256: item.identitySha256, contentType: item.contentType, version_prefix: versionPrefix,
    }));
  const manifest = {
...
async function writeProductionPhaseResult(manifestUpload) {
  if (!emitPhaseResult) return;
  if (!execute || targetEnv !== "production") throw new Error("production_materialization_phase_result_requires_execute");
  ...
  const phaseResult = {
    schema: "ikimon.production-phase-result/v1", phase: "materialize", status: "succeeded",
    source_sha: materializationSourceSha, job_id: materializationJobId,
    bundle_hash: bundleHash, manifest_hash: manifestHash,
...
if (execute && directR2 && impactReceipt?.reuse_decision === "REUSE_EXACT") {
  ...
  manifestUpload = {
    ok: true, skipped: true, reason: skipReason, key: provenance.manifest_key,
    sha256: provenance.manifest_sha256 || null, versionPrefix: provenance.version_prefix,
    pointerKey: provenance.pointer_key, transport: "wrangler-r2",
  };
  events.push({ command: "reuse exact derived UI artifact from binding receipt", exitCode: 0,
    durationMs: 0, updated: 0, skipped: 0, failed: 0, uiR2PutCount: 0 });
  await app.close();
  await rm(tempDir, { recursive: true, force: true });
  const result = {
    ok: true, sourceSha: materializationSourceSha, mode: "execute", r2WritesRequested: true,
    r2WritesExecuted: false, bucket, targetEnv, scope, transport: "wrangler-r2", concurrency,
    skipIfUnchanged, impactReceipt: { locator: impactReceiptPath, receiptDigest: impactReceipt.receipt_digest,
      reuseDecision: impactReceipt.reuse_decision, artifactIdentityDigest: impactReceipt.artifact_identity_digest },
    manifestKey, bundleHash, materializeSkipped, skipReason, previousManifest: null,
    manifestUpload, uploadSummary, rendered: [], renderedStatic: [], events,
  };
  const resultText = `${JSON.stringify(result, null, 2)}\n`;
  if (outputPath) await writeFile(outputPath, resultText, "utf8");
  console.log(resultText);
  await writeProductionPhaseResult(manifestUpload);
  process.exit(0);
}
  ...
  if (execute) {
    ...
    if (directR2) {
      ...
      if (state.same_manifest) {
        ...
        uploadSummary.uiR2PutCount = pending.length;
