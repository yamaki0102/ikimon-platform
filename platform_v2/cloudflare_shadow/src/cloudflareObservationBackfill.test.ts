...
test("backfill is deterministic, idempotent and does not infer an accepted identification", async () => {
  const input = {
    ...
    identifications: [{ identification_id: "ident-1", occurrence_id: "occ:record-1:0", actor_user_id: "community-1", proposed_name: "アマガエル", proposed_rank: "species", stance: "support", source_key: "source-ident-1", source_payload_json: "{}", is_current: 1 }],
...
test("ambiguous ownership and missing parents are quarantined without provenance promotion", async () => {
  const plan = await buildRecordObservationBackfillPlan({
    ...
    identifications: [{ identification_id: "ident-orphan", occurrence_id: "occ:missing:0", actor_user_id: "user-1", proposed_name: "不明", proposed_rank: null, stance: "support", source_key: "orphan", source_payload_json: "{}", is_current: 1 }],
