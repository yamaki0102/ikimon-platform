...
function fallbackRoutePattern(pathname: string): string {
  ...
  if (/^\/ja\/community\/fields\/[^/]+$/.test(pathname)) return "/ja/community/fields/:id";
  if (/^\/ja\/places\/[^/]+\/snapshot$/.test(pathname)) return "/ja/places/:id/snapshot";
  if (/^\/ja\/observations\/[^/]+$/.test(pathname)) return "/ja/observations/:id";
...
function isOriginalUiHtmlPath(pathname: string): boolean {
  ...
  if (/^\/(?:ja|en|es|pt-br)\/observations\/[a-zA-Z0-9][a-zA-Z0-9_-]{0,127}$/.test(pathname)) return true;
  return false;
}
