...
function renderLearning(card: ObservationFirstCard, copy: ObservationFirstRecordDetailCopy): string {
  ...
  return `<div class="of-learning">
    ${visible.length ? `<section><h3>${escapeHtml(copy.distinguishingPoints)}</h3><ul>${visible.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></section>` : ""}
    ${advice.length ? `<section><h3>${escapeHtml(copy.shootingAdvice)}</h3><ul>${advice.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></section>` : ""}
  </div>`;
}
...
function renderOwnerManagement(
  ...
  const policy = detail.visibility !== "private"
    ...
    : "";
  return `<details class="of-manage" id="manage"><summary>${escapeHtml(copy.manage)}</summary><div class="of-manage-body">${add}${cards}${mediaAssignment}${policy}</div></details>`;
}
...
function renderRelated(items: ObservationFirstRelatedPresentation[] | undefined, lang: ObservationRecordLang, copy: ObservationFirstRecordDetailCopy): string {
  ...
  const cards = items.slice(0, 6).map((item) => {
    ...
    return `<a class="of-related-card" href="${escapeHtml(`${langPrefix(lang)}/observations/${encodeURIComponent(item.recordId)}`)}">${safePhoto ? `<img src="${escapeHtml(safePhoto)}" alt="" loading="lazy" decoding="async">` : ""}<span><strong>${escapeHtml(title)}</strong><small>${escapeHtml(item.observedLabel)}</small></span></a>`;
  }).join("");
