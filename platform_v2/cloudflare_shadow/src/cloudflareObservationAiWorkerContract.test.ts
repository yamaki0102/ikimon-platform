...
test("photo upload, queue, cron, Gemini Batch, and review target form one durable reassessment path", async () => {
  ...
  assert.match(source, /WITH latest_public AS \([\s\S]+FROM readmodel_public_observations r\s+JOIN observations o ON o\.observation_id = r\.observation_id[\s\S]+ORDER BY r\.observed_at DESC, o\.observation_id DESC\s+LIMIT 30/);
