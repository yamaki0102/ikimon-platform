...
function sourcePublicUrl(source: LlmoSource, origin: string): string {
  try {
    ...
    if (parsed.hostname === "ikimon.life" || parsed.hostname === "www.ikimon.life" || parsed.hostname === "staging.ikimon.life") {
      return `${origin}${parsed.pathname}${parsed.search}${parsed.hash}`;
    }
  } catch {
    // Static source definitions are validated by the generated output tests.
  }
...
function buildFromSources(title: string, purpose: string, sources: LlmoSource[], origin = resolveConfiguredPublicOrigin()): string {
  return [
    `# ${title}`,
    "",
    purpose,
...
function buildFromSourcesWithLimit(title: string, purpose: string, sources: LlmoSource[], maxChars: number, origin = resolveConfiguredPublicOrigin()): string {
  return [
    `# ${title}`,
    "",
    purpose,
...
export function buildLlmsTxt(origin = resolveConfiguredPublicOrigin()): string {
...
export function buildLlmoTermsMarkdown(origin = resolveConfiguredPublicOrigin()): string {
...
export function buildLlmoGuideMarkdown(origin = resolveConfiguredPublicOrigin()): string {
...
export function buildLlmoFaqMarkdown(origin = resolveConfiguredPublicOrigin()): string {
  return buildFromSources(
    "ikimon.life FAQ",
...
export function buildLlmoResearcherMarkdown(origin = resolveConfiguredPublicOrigin()): string {
  return buildFromSources(
