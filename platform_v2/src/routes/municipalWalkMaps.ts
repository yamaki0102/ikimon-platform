...
function isPublicVisible(config: MunicipalWalkMapConfigV0): boolean {
...
async function loadWalkMapConfig(
  ...
  if (dbConfig) {
    if (!options.allowDraft && !isPublicVisible(dbConfig)) return null;
  ...
  if (staticConfig && options.allowStaticFallback && (options.allowDraft || isPublicVisible(staticConfig))) {
...
export async function registerMunicipalWalkMapRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Body: unknown }>("/api/v1/municipal-walk-maps", async (request, reply) => {
    try {
      const access = await assertMunicipalWalkMapAdminAccess(request);
      const config = await prepareConfigForWrite(extractConfigFromBody(request.body));
      const saved = await upsertMunicipalWalkMapConfigV0(config, access.actorUserId ?? "system_write_key");
      return reply.status(201).send({ ok: true, config: saved, publicMap: buildMunicipalWalkMapPublicReadModelV0(saved), deprecated: true });
    } catch (error) {
      const message = error instanceof Error ? error.message : "municipal_walk_map_save_failed";
      reply.code(adminErrorStatus(message));
      return { ok: false, error: message };
    }
  });
...
export const municipalWalkMapRouteContract = {
  ...
  deprecatedCreateApiPath: "/api/v1/municipal-walk-maps",
