  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.equal(response.statusCode, 200);
        assert.match(response.body, /風景を残す/);
        assert.match(response.body, /写真・メモから始める/);
        assert.match(response.body, /地図や公開された発見はこのまま見られます。/);
        assert.match(response.body, /ログインして残す/);
        assert.match(response.body, /自分の記録と場所に残ります。/);
        assert.match(response.body, /href="\/ja\/map">地図を見る/);
        assert.match(response.body, /href="\/ja\/records\?view=public">みんなの発見/);
        assert.match(response.body, /あとで戻れる/);
        assert.match(response.body, /写真なしでも残せる/);
        assert.match(response.body, /公開前に確認できる/);
