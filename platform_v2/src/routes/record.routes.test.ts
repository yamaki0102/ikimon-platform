  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.match(response.body, /保存前の見え方/);
        assert.match(response.body, /id="record-prepublish-checklist"/);
        assert.match(response.body, /record-prepublish-private/);
        assert.match(response.body, /record-prepublish-public/);
        assert.match(response.body, /record-prepublish-media/);
        assert.match(response.body, /正確な地点は自分の整理に使います。/);
        assert.match(response.body, /地点なしでも保存できます。あとで足せます。/);
        assert.match(response.body, /候補になります。地図では広い範囲で表示します。/);
        assert.match(response.body, /処理完了後に候補になります。/);
        assert.match(response.body, /表示されません。/);
        assert.match(response.body, /写真の写り込みや一部情報で場所が分かる場合があります。/);
        assert.match(response.body, /selectedMediaKindText/);
        assert.match(response.body, /prepublishPublicTextForState/);
        ...
        assert.doesNotMatch(response.body, /record-prepublish-(?:private|public|media)[\s\S]{0,160}(?:latitude|longitude|toFixed\(6\))/);
  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.match(response.body, /Before saving/);
        assert.match(response.body, /The exact place is saved for your sorting/);
        assert.match(response.body, /Candidate\. Maps use a broad view/);
        assert.match(response.body, /Not shown/);
        assert.match(response.body, /Visible clues or some photo data can still reveal a place/);
