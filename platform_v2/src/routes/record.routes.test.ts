  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.match(response.body, /写真なしでも、聞こえた音や周囲の様子をメモとして保存できます。あとで写真や名前を足せます。/);
        assert.match(response.body, /名前が分からなくても、聞こえた音や場所メモから保存できます。写真はあとで足せます。/);
        assert.match(response.body, /見つからなかったことと、その時の音や場所の様子をメモとして保存できます。次に同じ場所で比べやすくなります。/);
        ...
        assert.match(response.body, /buildRecordReturnHtml/);
        assert.match(response.body, /publicStateSuccessKind/);
        assert.match(response.body, /const scrollStatusIntoView = \(\) =>/);
        assert.match(response.body, /この場所の記録が増えました。/);
        assert.match(response.body, /自分の記録に残り、地図でこの場所を見返せます。/);
        assert.match(response.body, /自分の記録から今すぐ見返せます。公開表示は確認後に反映されます。/);
        assert.match(response.body, /自分だけの記録として残りました。公開地図には出ません。/);
        assert.match(response.body, /data-record-success-cta/);
        assert.match(response.body, /map_nearby/);
        assert.match(response.body, /key: 'observation_detail', label: recordUiCopy\.successDetailCta, primary: true/);
        assert.match(response.body, /key: 'notes', label: recordUiCopy\.successMineCta, primary: false/);
        assert.match(response.body, /state !== 'hidden' && links\.mapHref/);
        assert.match(response.body, /const successReturnState = publicStateSuccessKind\(observationJson \|\| \{\}\);/);
  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.match(response.body, /Save what you heard or noticed as a note even without a photo\. Add photos or names later\./);
        ...
        assert.match(response.body, /This place now has one more record\./);
        assert.match(response.body, /It is in your records, and the map can take you back to this place\./);
        assert.match(response.body, /View my records/);
        assert.match(response.body, /Open this record/);
        assert.match(response.body, /View nearby map/);
        assert.match(response.body, /Record here again/);
