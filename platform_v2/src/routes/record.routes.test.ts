  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.match(response.body, /地図での見え方/);
        ...
        assert.match(response.body, /locationPrivacyPublicSet/);
        assert.match(response.body, /safeLat < -90 \|\| safeLat > 90 \|\| safeLng < -180 \|\| safeLng > 180/);
        assert.match(response.body, /Math\.max\(-85, Math\.min\(85, safeLat\)\)/);
        assert.match(response.body, /syncLocationPrivacyNotice/);
        assert.match(response.body, /syncRecordMapMarker/);
        assert.match(response.body, /new window\.maplibregl\.Marker\(\{ color: '#047857' \}\)\.setLngLat\(lngLat\)\.addTo\(recordMap\)/);
  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        ...
        assert.match(response.body, /Choose a place to preview the public map view below/);
