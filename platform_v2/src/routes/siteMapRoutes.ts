...
function requestOrigin(
  ...
  return resolvePresentationPublicOrigin(request, { explicitOrigin: publicAssetOrigin }) ?? STAGING_PUBLIC_ORIGIN;
}
...
export function isStagingRequest(
  ...
  return requestOrigin(request, publicAssetOrigin) === STAGING_PUBLIC_ORIGIN;
}
...
export async function registerSiteMapRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/sitemap.xml", async (request, reply) => {
    ...
    return buildXmlSitemap(requestOrigin(request as unknown as { headers: Record<string, unknown>; protocol?: string }));
  });
  ...
  app.get("/robots.txt", async (request, reply) => {
    ...
    return buildRobotsTxt(requestOrigin(request as unknown as { headers: Record<string, unknown>; protocol?: string }));
  });
  ...
  app.get("/qa/reflection-loop.json", async (request, reply) => {
    ...
    return buildReflectionLoopManifest(requestOrigin(request as unknown as { headers: Record<string, unknown>; protocol?: string }));
  });
}
