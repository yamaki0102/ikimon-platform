...
const EDIT_SESSION: ObservationEventSessionRow = {
  sessionId: "session-a", legacyEventId: null, eventCode: "ACT123", title: "地域の記録会",
  organizerUserId: "organizer-a", corporationId: null, plan: "community", primaryMode: "discovery",
  activeModes: ["discovery"], locationLat: null, locationLng: null, locationRadiusM: 1000,
  startedAt: "2026-09-02T01:00:00.000Z", endedAt: null, targetSpecies: [], config: {}, fieldId: "field-a",
  templateSourceSessionId: null, createdAt: "2026-09-01T05:00:00.000Z", updatedAt: "2026-09-01T05:00:00.000Z",
};
...
test("activation denies cross-origin and anonymous callers, then requires its key and semantic fields", async () => {
  const previous = { nodeEnv: process.env.NODE_ENV, enabled: process.env.ENABLE_DEV_DUMMY_ADMIN, token: process.env.DEV_DUMMY_ADMIN_TOKEN };
  process.env.NODE_ENV = "test"; process.env.ENABLE_DEV_DUMMY_ADMIN = "1"; process.env.DEV_DUMMY_ADMIN_TOKEN = "activation-route-test";
  const app = Fastify();
  await registerObservationEventApiRoutes(app); await app.ready();
  const sameOriginHeaders = { host: "localhost:3200", origin: "http://localhost:3200", "sec-fetch-site": "same-origin", "content-type": "application/json" };
  const organizerHeaders = { ...sameOriginHeaders, cookie: "ikimon_v2_session=activation-route-test" };
  try {
    const crossOrigin = await app.inject({ method: "POST", url: "/api/v1/observation-events", headers: { ...organizerHeaders, origin: "https://attacker.example", "sec-fetch-site": "cross-site" }, payload: {} });
    assert.equal(crossOrigin.statusCode, 403); assert.match(crossOrigin.body, /same_origin_required/);
    const anonymous = await app.inject({ method: "POST", url: "/api/v1/observation-events", headers: sameOriginHeaders, payload: {} });
    assert.equal(anonymous.statusCode, 401); assert.deepEqual(anonymous.json(), { error: "login required" });
    const missingKey = await app.inject({ method: "POST", url: "/api/v1/observation-events", headers: organizerHeaders, payload: {} });
    assert.equal(missingKey.statusCode, 400); assert.deepEqual(missingKey.json(), { error: "event_code activation key required" });
    const missingStart = await app.inject({ method: "POST", url: "/api/v1/observation-events", headers: organizerHeaders, payload: { event_code: "ACT123" } });
    assert.equal(missingStart.statusCode, 400); assert.deepEqual(missingStart.json(), { error: "started_at required" });
    const missingTitle = await app.inject({ method: "POST", url: "/api/v1/observation-events", headers: organizerHeaders, payload: { event_code: "ACT123", started_at: "2026-09-02T01:00:00.000Z" } });
    ...
    if (previous.enabled === undefined) delete process.env.ENABLE_DEV_DUMMY_ADMIN; else process.env.ENABLE_DEV_DUMMY_ADMIN = previous.enabled;
    if (previous.token === undefined) delete process.env.DEV_DUMMY_ADMIN_TOKEN; else process.env.DEV_DUMMY_ADMIN_TOKEN = previous.token;
  }
});
...
test("legacy sessions without a code remain editable while established invite codes stay immutable", () => {
  ...
  const established = renderEventEditBody({ session: { ...EDIT_SESSION, eventCode: "miyakoda-summer" }, strings });
