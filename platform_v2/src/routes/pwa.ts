...
export async function registerPwaRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get<{ Querystring: { lang?: string } }>("/debug/app-outbox", async (request, reply) => {
    ...
    return renderSiteDocument({
      ...
      description: "端末内に保存されたZUKAN app outboxの状態確認ページ。",
      activeNav: "",
      body,
...
function safeAppRefreshTarget(value: unknown): string {
  ...
  try {
    const parsed = new URL(raw, "https://zukan.earth");
    if (parsed.origin !== "https://zukan.earth") return "/map";
