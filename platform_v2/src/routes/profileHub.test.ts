...
test("self profile hub deduplicates repeated regional story cards before rendering", () => {
  ...
  assert.equal((html.match(/同じ場所を見るなら、道の端を残す。/g) ?? []).length, 1);
  assert.equal((html.match(/別の場所を見るなら、草地の境目を残す。/g) ?? []).length, 1);
});
