...
test("app service worker is separate from legacy cleanup worker and caches app shells only as fallback", async () => {
  ...
  try {
    ...
    assert.match(response.body, /RECORD_NAV_RE/);
    assert.match(response.body, /MAP_NAV_RE/);
    assert.match(response.body, /PERSONAL_NAV_RE/);
    assert.match(response.body, /REFRESH_NAV_RE/);
    assert.doesNotMatch(response.body, /REFRESH_NAV_RE[\s\S]*record\\\/\?\$/);
    assert.doesNotMatch(response.body, /REFRESH_NAV_RE[\s\S]*settings/);
    assert.match(response.body, /REFRESH_NAV_RE[\s\S]*records/);
    ...
    assert.match(response.body, /const isRecordShell = RECORD_NAV_RE\.test\(path\)/);
    assert.match(response.body, /isRecordShell \|\| isMapShell \|\| isPersonalShell/);
    assert.match(response.body, /&& !isRecordShell && !isMapShell && !isPersonalShell/);
    const refreshNavMatch = response.body.match(/const REFRESH_NAV_RE = (\/.+\/);/);
