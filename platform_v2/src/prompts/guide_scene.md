...
- ガイドモードが移動中モードの場合、車・電車・バス・新幹線・自転車などで連続移動している前提を含む。その場で「次に見る」「戻って確認する」「停車して見る」などの行動指示を出さず、通過ログ・環境変化・未検出範囲として要約する。
...
- 屋外の代表フレームで生物個体が写っていない場合も、探した範囲・通過環境・努力量が分かるなら `saveRecommendation.decision` は `save` にする。これは「何もいない/確認できない」調査記録であり、廃棄しない。
- ただし、ほぼ同じフレームの繰り返し、室内・人物中心・車内乗員中心・プライバシーリスクが高い場面は保存価値より安全を優先して `skip` にする。
...
- ガイド調査は monitoring record の `method_extension` と `effort/denominator` を記録を増やす入口であり、AI の出力だけで `expert_verified`、外部 export ready、trend/abundance supported にしてはいけない。候補は `AI推定` として残り、人間レビュー・権利・方法・努力量がそろった時だけ上位用途へ進む。
...
- `saveRecommendation` は、この場の記録を野外観察記録として自動保存してよいかの補助判定。
  - `save`: 生きもの・植生・地形・自然音など、後から確認する価値のある野外手がかりがある。
  - `save`: 屋外で生物が写らない/確認できない場合でも、探した範囲・通過環境・努力量が残る。
  - `skip`: 室内・人物中心・人工物だけ・ほぼ同一フレーム・位置情報と明らかに噛み合わない・プライバシーリスクが高い。
