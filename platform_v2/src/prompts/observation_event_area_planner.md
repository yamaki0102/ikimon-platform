...
- 最初と最後の座標は同じにして閉じる。
- 入力中心から極端に離れた範囲を作らない。
- ユーザーが指定した drawn_polygon を基本範囲として扱い、各候補はその範囲の主要部分をカバーする。小さな集合場所だけに縮めすぎない。
- 3候補は役割差を明確にする。facility は集合場所と建物周辺、safe_walk は歩きやすさ、nature_rich は緑地・水辺・観察価値を優先する。
- 「正しい境界」「公式境界」などの断定は禁止。
