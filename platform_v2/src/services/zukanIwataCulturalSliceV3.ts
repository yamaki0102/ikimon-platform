...
export const ZUKAN_IWATA_CULTURAL_SLICE_V3_SCHEMA =
  "zukan.iwata-cultural-slice/v3" as const;
...
export type IwataCulturalClaimField = "name" | "address" | "summary";

export type IwataCulturalReviewV3 = {
...
export type IwataCulturalSliceV3Input = {
  tenantId: string;
  sourceRecordId: string;
  review?: IwataCulturalReviewV3 | null;
};

export type IwataIdentityLinkCandidateV3 = {
  state: "candidate";
...
export type IwataPublicationCandidateV3 = {
...
export type IwataCulturalSliceV3Plan = {
  schema: typeof ZUKAN_IWATA_CULTURAL_SLICE_V3_SCHEMA;
  mode: "shadow_only";
  tenantId: string;
  ...
  viewCandidate: {
    ...
    identityCandidates: IwataIdentityLinkCandidateV3[];
  };
  publicationCandidates: IwataPublicationCandidateV3[];
  gates: string[];
  blockers: string[];
  warnings: string[];
...
function buildClaim(input: {
  ...
  sourceEditionId: string;
  review: IwataCulturalReviewV3 | null;
  approved: ReadonlySet<IwataCulturalClaimField>;
  ...
  return {
    ...
    predicateUri: `https://zukan.earth/predicate/${input.field}`,
    predicateVersion: 1,
    value: input.value,
...
function buildClaims(input: {
  ...
  const approved = new Set<IwataCulturalClaimField>(
    input.review ? uniqueSorted(input.review.approvedFields) as IwataCulturalClaimField[] : [],
  );
...
function buildPublication(input: {
  tenantId: string;
  outputKind: IwataPublicationCandidateV3["outputKind"];
  envelope: RegionalKnowledgeEnvelopePlan;
  review: IwataCulturalReviewV3;
}): IwataPublicationCandidateV3 {
...
export function planIwataCulturalSliceV3(
  input: IwataCulturalSliceV3Input,
): IwataCulturalSliceV3Plan {
  ...
  const envelope = planRegionalKnowledgeEnvelope({
    ...
    payload: {
      ...
      sourceUpdatedAt: canonicalTimestamp(item.sourceUpdatedAt),
      retrievedAt: canonicalTimestamp(IWATA_OPEN_DATA_RETRIEVED_AT),
      sourceValues: {
        name: item.name,
  ...
  const planWithoutDigest = {
    schema: ZUKAN_IWATA_CULTURAL_SLICE_V3_SCHEMA,
