...
test("taxon insight cache and generation require a Latin scientific name", () => {
  ...
  assert.match(source, /if \(!sn\) return empty\(sn, vn\);/);
  assert.match(source, /const cacheKey = sn;/);
  assert.doesNotMatch(source, /const cacheKey = sn \|\| vn;/);
});
