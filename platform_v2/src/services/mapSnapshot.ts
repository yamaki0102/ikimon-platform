...
const PUBLIC_MAP_HAS_PRESENTABLE_PHOTO_SQL = `
  exists (
    select 1
     ...
     where public_photo_ea.visit_id = v.visit_id
       and ${mapPresentablePhotoAssetSql("public_photo_ea", "public_photo_ab")}
  )
`;
...
async function fetchPublicMapRows(filters: MapQueryFilters): Promise<{
  ...
  const whereClauses: string[] = [
    ...
    PUBLIC_OBSERVATION_QUALITY_SQL,
    PUBLIC_MAP_HAS_PRESENTABLE_PHOTO_SQL,
  ];
...
export function buildPublicCellRecords(
  ...
  const items = sorted
    ...
    .map((entry) => {
      const viewerOwnsRecord = Boolean(filters.viewerUserId && entry.row.userId === filters.viewerUserId);
      return {
