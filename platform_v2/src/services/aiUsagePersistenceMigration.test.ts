...
test("AI usage migrations are isolated from ai_cost_log and Foundation", () => {
  const combined = migrationFiles.map(sql).join("\n");
...
test("v2 migrations enforce scope invocation fencing and provider request idempotency", () => {
  const v2 = `${sql("0143_ai_usage_contract_v2.sql")}\n${sql("0144_ai_usage_invocation_identity.sql")}`;
  ...
  assert.match(v2, /invocation_id TEXT NOT NULL/u);
