...
test("malformed lifecycle timestamps never expose an existing public URL", () => {
  for (const field of ["publicDerivativeVerifiedAt", "publicReadyAt"] as const) {
    for (const value of ["not-a-timestamp", "2026-02-31T00:00:00.000Z", "2026-01-01T24:00:00.000Z"]) {
      const malformed = decideZukanMediaPrivacyPolicy({ ...verifiedDerivative, [field]: value });
      assert.equal(malformed.canExposePublicMedia, false, `${field}:${value}`);
      assert.equal(malformed.publicMediaUrl, null, `${field}:${value}`);
    }
  }
});
...
test("unapproved schemes, hosts, and paths never become public media URLs", () => {
  for (const publicMediaUrl of [
    "javascript:alert(1)",
    "data:text/plain,unsafe",
    "https://evil.example/derived/unsafe.webp",
    "https://user:password@ikimon.life/derived/unsafe.webp",
    "https://ikimon.life:444/derived/unsafe.webp",
    "/private/observation.webp",
    "/derived/../private/observation.webp",
  ]) {
    const result = decideZukanMediaPrivacyPolicy({ ...verifiedDerivative, publicMediaUrl });

    assert.equal(result.canExposePublicMedia, false, publicMediaUrl);
    assert.equal(result.publicMediaUrl, null, publicMediaUrl);
    assert.equal(result.reason, "public_media_missing", publicMediaUrl);
  }
});
