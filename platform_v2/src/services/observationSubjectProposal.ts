...
export type ProposeObservationSubjectFromCandidateResult = {
  visitId: string;
  occurrenceId: string;
...
export async function proposeObservationSubjectFromCandidate(
  input: ProposeObservationSubjectFromCandidateInput,
): Promise<ProposeObservationSubjectFromCandidateResult> {
  ...
  try {
    ...
    if (candidate.candidate_status === "dismissed") throw new Error("candidate_dismissed");
    ...
    const sourcePayload = {
      source: "community_subject_proposal",
    ...
    if (candidate.observer_user_id && candidate.observer_user_id !== input.actorUserId) {
      ...
      await client.query(
        `insert into alert_deliveries (
            ...
            delivery_status, delivered_at, payload_json, created_at
         ) values (
            $1, $2, 'subject_proposal', 'in_app',
            'sent', now(), $3::jsonb, now()
         )`,
        [
          occurrenceId,
          candidate.observer_user_id,
          JSON.stringify({
            title: "写真・動画に別の対象が提案されました",
            body: `${proposedName}も写っているかもしれません。名前や反対意見を確認できます。`,
            occurrenceId,
            visitId: candidate.visit_id,
            candidateId: candidate.candidate_id,
            actorUserId: input.actorUserId,
            label: proposedName,
            summary: "投稿者の正式な主張ではなく、この写真・動画を見た人からの提案です。",
          }),
        ],
      );
    }
