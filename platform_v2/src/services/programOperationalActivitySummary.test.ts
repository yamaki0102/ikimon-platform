...
test("regional view references project only the allowed fields", () => {
  const extraReference = {
    referenceId: "view-extra",
    placeId: "place-1",
    status: "candidate",
    taxon: "forbidden",
    speciesCount: 4,
    biodiversity: "forbidden",
    reportReady: true,
  } as unknown as NonNullable<OperationalActivitySummaryInput["regionalViewReferences"]>[number];
  const summary = buildProgramOperationalActivitySummary(fixture({
    regionalViewReferences: [extraReference],
  }));
  const references = summary.publication.regionalViewReferences;

  assert.deepEqual(references, [{ referenceId: "view-extra", placeId: "place-1", status: "candidate" }]);
  assert.doesNotMatch(JSON.stringify(summary), /taxon|species|biodiversity|report|inventory|composition|comparison|csv|excel|pdf/i);
});
