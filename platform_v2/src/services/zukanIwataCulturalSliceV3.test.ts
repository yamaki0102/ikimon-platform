...
import {
  planIwataCulturalSliceV3,
  type IwataCulturalReviewV3,
} from "./zukanIwataCulturalSliceV3.js";

const tenantId = "tenant-zukan-iwata-cultural-v3";
...
test("unreviewed real cultural Record produces no Publication", () => {
  const plan = planIwataCulturalSliceV3({ tenantId, sourceRecordId });
  ...
  assert.equal(plan.envelope.claims.length, 2);
...
test("SourceEdition timestamps remain provenance, not Entity Claims", () => {
  const plan = planIwataCulturalSliceV3({
    tenantId,
    sourceRecordId,
    review: review(),
  });
  const predicates = plan.envelope.claims.map((claim) => claim.predicateUri);
  const payload = JSON.parse(plan.envelope.record.payloadJson) as Record<string, unknown>;

  assert.deepEqual(predicates.sort(), [
    "https://zukan.earth/predicate/name",
    "https://zukan.earth/predicate/summary",
  ]);
  assert.ok(!predicates.some((predicate) => predicate.includes("source-updated")));
  assert.equal(payload.sourceUpdatedAt, "2024-03-26T00:00:00.000Z");
  assert.equal(payload.retrievedAt, "2026-07-28T00:00:00.000Z");
});
...
test("reviewed Claims generate two outputs while persistence remains dry-run", () => {
  ...
  assert.deepEqual(plan.blockers, []);
  assert.equal(plan.persistence.writeEnabled, false);
  assert.equal(plan.persistence.counts.claims, 2);
  assert.equal(plan.persistence.counts.claimRevisions, 2);
  assert.equal(plan.persistence.counts.claimRecordLinks, 2);
...
test("equivalent approved-field order produces the same plan", () => {
  ...
  const first = planIwataCulturalSliceV3({ tenantId, sourceRecordId, review: firstReview });
  const second = planIwataCulturalSliceV3({ tenantId, sourceRecordId, review: secondReview });
...
test("invalid review identity and rights references fail closed", () => {
  ...
  const plan = planIwataCulturalSliceV3({ tenantId, sourceRecordId, review: invalid });
...
test("unknown source Record fails closed", () => {
  assert.throws(
    () => planIwataCulturalSliceV3({ tenantId, sourceRecordId: "UNKNOWN" }),
    /iwata_cultural_source_record_not_found/,
  );
});

test("cultural slice does not reuse biodiversity semantics", () => {
  const serialized = JSON.stringify(planIwataCulturalSliceV3({
    tenantId,
    sourceRecordId,
    review: review(),
  }));
