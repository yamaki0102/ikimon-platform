...
const GITHUB_REPOSITORY_URL = "https://github.com/yamaki0102/ikimon-platform.git";
const GITHUB_CREDENTIAL_HELPER = "!gh auth git-credential";

export function buildGitHubGitArgs(args: string[]): string[] {
...
export async function receiveProposal(submission: ProposalSubmission): Promise<ProposalResult> {
  ...
  // Refresh main over HTTPS while keeping the PAT out of process arguments.
  // `gh auth git-credential` reads GH_TOKEN from the child environment.
  await runGit(
    buildGitHubGitArgs(["fetch", "--no-tags", GITHUB_REPOSITORY_URL, "main:refs/remotes/origin/main"]),
    REPO_ROOT,
    buildGitHubGitEnv(ghToken),
  );
  ...
  try {
    ...
    // 6. Push over HTTPS with the same environment-only credential path.
    await runGit(
      buildGitHubGitArgs(["push", "--force-with-lease", GITHUB_REPOSITORY_URL, `${branchName}:${branchName}`]),
      worktreePath,
      buildGitHubGitEnv(ghToken),
    );
