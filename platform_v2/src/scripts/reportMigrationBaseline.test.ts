...
test("db migration baseline rehearsal locks the current migration head and risk inventory", async () => {
  ...
  assert.equal(report.totalMigrations, 135);
  assert.equal(report.firstMigration, "0001_extensions_and_core.sql");
  assert.equal(report.headMigration, "0132_atomic_observation_photo_processing_intent.sql");
  ...
  assert.equal(report.riskSummary.ownerSensitiveApproved, 34);
