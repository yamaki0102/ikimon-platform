...
test("db migration baseline rehearsal locks the current migration head and risk inventory", async () => {
  ...
  assert.equal(report.totalMigrations, 125);
  assert.equal(report.firstMigration, "0001_extensions_and_core.sql");
  assert.equal(report.headMigration, "0123_municipal_walk_maps.sql");
  ...
  assert.equal(report.riskSummary.ownerSensitiveApproved, 30);
