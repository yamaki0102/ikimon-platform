...
(function () {
  ...
  function isIkimonSourceUrl(url) {
    ...
    try {
      ...
      return parsed.hostname === 'zukan.earth' || /\\.zukan\\.earth$/.test(parsed.hostname)
        || parsed.hostname === 'ikimon.life' || /\\.ikimon\\.life$/.test(parsed.hostname);
    } catch (_) {
      return /^https?:\\/\\/(?:[^/]+\\.)?(?:zukan\\.earth|ikimon\\.life)(?:[/:?#]|$)/i.test(text);
    }
  }
