...
function displayName(item: LandingObservation, copy: LandingStrings): string {
  if (item.isAiCandidate && !item.vernacularName && !item.scientificName) return copy.home.shared.unknownRecord;
...
function renderMedia(item: LandingObservation, copy: LandingStrings, eager = false): string {
  ...
  if (imageUrl) {
    return `<span class="home-card-media is-${kind}"><img src="${escapeHtml(imageUrl)}" alt="" width="680" height="510" loading="${eager ? "eager" : "lazy"}" decoding="async"${eager ? ' fetchpriority="high"' : ""} />${kind === "video" ? `<span class="home-media-affordance">${mediaIcon("video")}<span>${escapeHtml(copy.home.shared.media.video)}</span></span>` : ""}${countBadge}</span>`;
  }
...
function renderRecentCard(options: LandingHomeStateOptions, item: LandingObservation, eager = false): string {
  ...
  return `<a class="home-recent-card" href="${escapeHtml(detailHref(options, item))}" data-home-record-id="${escapeHtml(observationKey(item))}">${renderMedia(item, options.copy, eager)}<span class="home-card-copy"><strong>${escapeHtml(displayName(item, options.copy))}</strong>${meta ? `<span>${escapeHtml(meta)}</span>` : ""}</span></a>`;
}
...
function renderGuestPhotoTile(options: LandingHomeStateOptions, item: LandingObservation, index: number): string {
  ...
  return `<a class="home-guest-proof-item is-item-${index + 1}" href="${escapeHtml(detailHref(options, item))}" data-home-public-record="${escapeHtml(observationKey(item))}"><img src="${escapeHtml(imageUrl)}" alt="" width="900" height="900" loading="${index === 0 ? "eager" : "lazy"}" decoding="async"${index === 0 ? ' fetchpriority="high"' : ""} /><span class="home-guest-proof-caption"><strong>${escapeHtml(name)}</strong>${meta ? `<small>${escapeHtml(meta)}</small>` : ""}</span></a>`;
}

function renderGuestProof(options: LandingHomeStateOptions, publicItems: LandingObservation[]): string {
  const photos = publicItems.filter((item) => Boolean(item.photoUrl) && homePreviewPolicy(item) !== "excluded").slice(0, 5);
  if (photos.length === 0) {
    return `<div class="home-guest-proof is-empty" aria-label="ZUKAN"><img src="/assets/brand/zukan-symbol.svg" alt="" width="220" height="220" /><span>${escapeHtml(options.copy.home.guest.placesTitle)}</span></div>`;
  }
...
function renderGuest(options: LandingHomeStateOptions, publicItems: LandingObservation[]): string {
  ...
  const placeItem = publicItems.find((item) => Boolean(item.photoUrl) && homePreviewPolicy(item) !== "excluded") ?? null;
  const categoryItems = copy.categories.map((item, index) => `<li><span class="home-category-index">${String(index + 1).padStart(2, "0")}</span><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.body)}</small></span></li>`).join("");
  ...
  return `<div class="home-state-view is-guest" data-home-view="guest"${options.isLoggedIn ? " hidden" : ""}><section class="home-guest-hero has-visual"><div class="home-guest-hero-copy"><span class="home-product-kicker">ZUKAN</span><h1>${renderHeroHeading(options.lang, copy.heroHeading)}</h1><p>${escapeHtml(copy.heroLead)}</p><div class="home-hero-actions">${captureButton(copy.primaryCta, "home-primary-button", "top_capture")}<a class="home-secondary-link" href="${escapeHtml(placeHref)}" data-kpi-event="top_place_tap" data-kpi-action="top_place">${escapeHtml(copy.secondaryCta)}</a></div></div>${slot("guest-hero", `<div class="home-guest-hero-visual">${renderGuestProof(options, publicItems)}</div>`)}</section><section class="home-section home-category-section"><div class="home-section-heading"><h2>${escapeHtml(copy.categoriesTitle)}</h2></div><ul>${categoryItems}</ul></section><section class="home-section home-value-section"><h2>${escapeHtml(copy.flowTitle)}</h2><ol>${flowItems}</ol></section><section class="home-section home-place-section" id="home-places">${placeVisual}<div><span class="home-product-kicker">PLACE</span><h2>${escapeHtml(copy.placesTitle)}</h2><p>${escapeHtml(copy.placesBody)}</p><a class="home-secondary-button" href="${escapeHtml(placeHref)}" data-kpi-event="top_place_tap" data-kpi-action="top_place_section">${escapeHtml(copy.secondaryCta)}</a></div></section><section class="home-section home-privacy-section"><span class="home-privacy-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="5" y="10" width="14" height="11" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/></svg></span><div><h2>${escapeHtml(copy.privacyTitle)}</h2><p>${escapeHtml(copy.privacyBody)}</p></div></section><section class="home-section home-final-section"><h2>${escapeHtml(copy.finalTitle)}</h2>${captureButton(copy.finalCta, "home-secondary-button", "top_capture_final")}</section></div>`;
}
...
function renderPlaceChangeCard(options: LandingHomeStateOptions, place: HomePlace, ownItems: LandingObservation[], p0: MemberP0Copy): string {
  ...
  const dates = [place.previousObservedAt, place.lastObservedAt].map((value) => formatShortDate(value, homeDateLocale(options.lang))).filter(Boolean);
  const comparison = dates.length >= 2 ? dates.join(" → ") : fillHomeTemplate(p0.comparableRecordsTemplate, "count", String(place.visitCount));
  return `<a class="home-place-change-card" href="${escapeHtml(cardHref)}" data-home-place-change="${escapeHtml(place.placeId)}">${record ? renderMedia(record, options.copy) : ""}<span class="home-place-change-copy"><strong>${escapeHtml(place.placeName)}</strong>${place.municipality ? `<span>${escapeHtml(place.municipality)}</span>` : ""}<small>${escapeHtml(comparison)}</small></span></a>`;
}

function renderHomeContinuationScript(viewerUserId: string): string {
  return `<script>(() => { const member = document.querySelector('[data-home-view="member"][data-home-draft-owner]'); const draftState = member && member.querySelector('[data-home-primary-state="draft_resume"]'); if (!member || !draftState || !('indexedDB' in window)) return; const ownerId = ${JSON.stringify(viewerUserId)}; if (!ownerId || member.getAttribute('data-home-draft-owner') !== ownerId) return; try { const request = indexedDB.open('ikimon-record-draft', 1); request.onsuccess = () => { const db = request.result; if (!db.objectStoreNames.contains('drafts')) { db.close(); return; } const tx = db.transaction('drafts', 'readonly'); const getRequest = tx.objectStore('drafts').get('latest:user:' + ownerId); getRequest.onsuccess = () => { const draft = getRequest.result; const files = draft && Array.isArray(draft.files) ? draft.files : []; const owned = draft && draft.ownerKey === 'user:' + ownerId; if (!owned || !(draft.file || files.length > 0 || (draft.metadata && draft.metadata.formValues))) return; member.querySelectorAll('[data-home-primary-state]').forEach((state) => { state.hidden = state !== draftState; state.setAttribute('data-home-primary-active', state === draftState ? 'true' : 'false'); }); }; tx.oncomplete = () => db.close(); tx.onerror = () => db.close(); }; } catch (_) {} })();</script>`;
}
...
function renderMember(options: LandingHomeStateOptions, ownItems: LandingObservation[]): string {
  ...
  const baseHero = (() => {
    if (memory) {
      ...
      return `<section class="home-member-primary is-memory${memory.photoUrl ? " has-photo" : ""}" data-home-primary-state="recent_memory" data-home-primary-active="true" data-home-record-id="${escapeHtml(observationKey(memory))}"><a class="home-member-primary-media" href="${escapeHtml(detailHref(options, memory))}" aria-label="${escapeHtml(copy.memoryCta)}">${renderMedia(memory, options.copy, true)}</a><div class="home-member-primary-copy"><span class="home-member-eyebrow">${escapeHtml(copy.memoryEyebrow)}</span><h1>${escapeHtml(displayName(memory, options.copy))}</h1>${meta ? `<p class="home-member-meta">${escapeHtml(meta)}</p>` : ""}<p>${escapeHtml(copy.memoryLead)}</p><a class="home-primary-button" href="${escapeHtml(detailHref(options, memory))}" data-kpi-action="home_memory_open">${escapeHtml(copy.memoryCta)}</a></div></section>`;
    }
    if (hasPrivateHistory) return `<section class="home-member-primary is-private-memory" data-home-primary-state="recent_memory" data-home-primary-active="true"><div class="home-member-primary-copy"><span class="home-member-eyebrow">${escapeHtml(copy.memoryEyebrow)}</span><h1>${escapeHtml(copy.recentTitle)}</h1><p>${escapeHtml(copy.memoryLead)}</p><a class="home-primary-button" href="${escapeHtml(href(options, "/records?view=mine"))}" data-kpi-action="home_memory_open">${escapeHtml(copy.memoryCta)}</a></div></section>`;
    return `<section class="home-member-primary is-first" data-home-primary-state="first_record" data-home-primary-active="true"><div class="home-member-primary-copy"><span class="home-product-kicker">ZUKAN</span><h1>${escapeHtml(copy.emptyTitle)}</h1><p>${escapeHtml(copy.emptyBody)}</p><div class="home-empty-actions">${captureButton(copy.primaryCta, "home-primary-button", "home_empty_capture")}${galleryButton(copy.galleryCta, "home-secondary-button")}</div><a class="home-secondary-link" href="${escapeHtml(href(options, "/map?tab=places"))}" data-kpi-event="top_place_tap" data-kpi-action="home_empty_place">${escapeHtml(copy.emptyPlaceCta)}</a></div></section>`;
  })();
  const recentItems = memory ? safeItems.slice(1, 5) : safeItems.slice(0, 4);
  const recentSection = recentItems.length > 0 ? `<section class="home-section home-recent-section"><div class="home-section-heading"><h2>${escapeHtml(copy.recentTitle)}</h2><a href="${escapeHtml(href(options, "/records?view=mine"))}">${escapeHtml(copy.recentCta)}</a></div><div class="home-recent-grid">${recentItems.map((item) => renderRecentCard(options, item)).join("")}</div></section>` : "";
  ...
  const sameSeasonItems = memorySeason ? pastPool.filter((item) => homeSeasonKey(item.observedAt) === memorySeason) : [];
  ...
  const pastSection = pastItems.length > 0 ? `<section class="home-section home-past-section"><h2>${escapeHtml(pastTitle)}</h2><div class="home-recent-grid">${pastItems.map((item) => renderRecentCard(options, item)).join("")}</div></section>` : "";
  const changePlaces = options.snapshot.myPlaces.filter((place) => Boolean(place.previousObservedAt) || place.visitCount > 1).slice(0, 4);
  const placesSection = changePlaces.length > 0 ? `<section class="home-section home-places-section"><div class="home-section-heading"><h2>${escapeHtml(p0.placeChangesTitle)}</h2><a href="${escapeHtml(href(options, "/map?tab=places"))}">${escapeHtml(copy.placesCta)}</a></div><div class="home-place-change-grid">${changePlaces.map((place) => renderPlaceChangeCard(options, place, safeItems, p0)).join("")}</div></section>` : "";
  const nextPlace = options.snapshot.myPlaces.find((place) => Boolean(place.nextLookFor || place.revisitReason)) ?? changePlaces[0] ?? null;
  const nextFocus = nextPlace ? pickPlaceFocus(nextPlace) : null;
  const nextSection = nextPlace ? `<section class="home-section home-next-section" data-home-next-action><h2>${escapeHtml(p0.nextActionTitle)}</h2><div class="home-next-action-card"><span><strong>${escapeHtml(nextPlace.placeName)}</strong><small>${escapeHtml(nextFocus ? fillHomeTemplate(p0.nextFocusTemplate, "focus", nextFocus) : p0.nextFallback)}</small></span><a href="${escapeHtml(buildPlaceRecordHref(options.basePath, options.lang, options.snapshot.viewerUserId, nextPlace))}" data-kpi-event="capture_nav_tap" data-kpi-action="home_place_revisit">${escapeHtml(p0.captureNow)}</a></div></section>` : "";
  const viewerUserId = options.snapshot.viewerUserId ?? "";
  return `<div class="home-state-view is-member" data-home-view="member" data-home-draft-owner="${escapeHtml(viewerUserId)}" data-home-base-state="${baseState}"${options.isLoggedIn ? "" : " hidden"}><section class="home-member-primary is-draft" data-home-primary-state="draft_resume" data-home-primary-active="false" hidden><div class="home-member-primary-copy"><span class="home-member-eyebrow">${escapeHtml(copy.continuationTitle)}</span><h1>${escapeHtml(copy.continuationTitle)}</h1><p>${escapeHtml(copy.continuationBody)}</p><a class="home-primary-button" href="${escapeHtml(href(options, "/record?draft=1&source=home_continue"))}">${escapeHtml(copy.continuationCta)}</a></div></section>${sectionSlot("member-primary", baseHero)}${renderHomeContinuationScript(viewerUserId)}${sectionSlot("member-recent", recentSection)}${sectionSlot("member-discovery", pastSection)}${sectionSlot("member-place", placesSection)}${sectionSlot("member-next", nextSection)}</div>`;
}
...
export function renderLandingHomeState(options: LandingHomeStateOptions): { heroHtml: string; bodyHtml: string } {
  ...
  const publicItems = unique([...(options.snapshot.publicProofFeed || []), ...options.snapshot.feed]).filter((item) => item.publicFeedEligible !== false && !ownKeys.has(observationKey(item)));
  return { heroHtml: `<div class="home-state-root" data-home-contract="state-split-v1" data-home-auth-state="${options.isLoggedIn ? "member" : "guest"}">${renderGuest(options, publicItems)}`, bodyHtml: `${renderMember(options, ownItems)}</div>` };
}
