import { renderQuietDiscoveryEntry, SAVED_REFERENCE_STYLES } from "./savedReferencesUi.js";
...
function renderMember(options: LandingHomeStateOptions, ownItems: LandingObservation[]): string {
  ...
  const baseState = "discovery";
  const baseHero = renderQuietDiscoveryEntry(options.basePath, options.lang);
  const recentItems = safeItems.slice(0, 4);
...
export const LANDING_HOME_STATE_STYLES = `${SAVED_REFERENCE_STYLES}
