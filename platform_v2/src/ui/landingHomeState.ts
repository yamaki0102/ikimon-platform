...
function canonicalPublicProofRecord(item: LandingObservation): LandingObservation {
  return {
    ...item,
    publicFeedGateStatus: item.publicFeedGateStatus ?? "public_eligible",
    publicFeedEligible: item.publicFeedEligible ?? true,
  };
}
...
export function renderLandingHomeState(options: LandingHomeStateOptions): { heroHtml: string; bodyHtml: string } {
  ...
  const publicProofItems = (options.snapshot.publicProofFeed || []).map(canonicalPublicProofRecord);
  const publicItems = unique([...publicProofItems, ...options.snapshot.feed])
