...
const copyByLang: Record<SiteLang, TodayHabitCopy> = {
  ja: {
    eyebrow: "今日のあなた",
    guestHeading: "今日の 1 件の記録が、続きをつくる",
    guestLead: "記録を始めると、今日／今週／次の 1 枚の理由がここに並ぶ。最初の記録から、同じ道の見え方が変わり始める。",
    guestCta: "最初の記録を残す",
    loggedInEyebrow: "今日のあなた",
    loggedInHeadingToday: (count) => `今日、${count} 件の記録を残せた。いい 1 日だ。`,
    loggedInHeadingNoToday: "今日はまだ、1 件も記録を残していない。",
    ...
    loggedInLeadFirstTime: "最初の記録を残せば、あなたの続きがここから立ち上がる。",
    weekLabel: (count) => `今週 ${count} 件`,
    streakLabel: (days) => `${days} 日続けて`,
    nudgeWriteToday: "今日の記録を残す",
    nudgeKeepStreak: (next) => `もう 1 件で、${next} 日続けて`,
    nudgeReturn: (gap) => `${gap} 日ぶりに記録を残す`,
    nudgeFirstPage: "最初の記録を残す",
    ctaContinue: "続きを残す",
    ctaReturn: "また残す",
    ctaStart: "始める",
  },
