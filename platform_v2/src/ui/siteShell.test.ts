...
test("site shell hydrates the login link from the v2 session endpoint", () => {
  ...
  assert.match(html, /href="\/ja\/records" title="発見を見る"/);
  assert.match(html, /desktop-side-nav-label">発見を見る/);
  assert.match(html, /href="\/ja\/records\?view=needs_id">名前を待つ記録/);
  assert.match(html, /href="\/ja\/map">地図を見る/);
  assert.match(html, /href="\/ja\/records\?view=public">みんなの発見/);
  assert.match(html, /href="\/ja\/register\?redirect=%2Frecord">残し始める/);
  assert.match(html, /href="\/ja\/login\?redirect=%2Frecord">ログイン/);
  assert.match(html, /href="\/ja\/record">今日残す/);
  assert.match(html, /href="\/ja\/records\?view=mine">自分の記録/);
  assert.match(html, /href="\/ja\/map\?tab=places">残した場所/);
  ...
  assert.match(html, /desktop-side-nav-section-title">まず見る/);
  assert.match(html, /desktop-side-nav-section-title">自分の続き/);
