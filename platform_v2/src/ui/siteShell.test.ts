...
test("site shell renders a global record footer nav outside the record flow", () => {
  ...
  assert.match(html, /requestCurrentLocationForDirectPost/);
  ...
  assert.match(html, /現在地を確認しています/);
  assert.match(html, /現在地を確認しました。記録を保存します/);
  assert.match(html, /現在地を確認できませんでした。記録画面で座標を確認して保存してください。/);
