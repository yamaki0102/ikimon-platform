...
const COPY: Record<SiteLang, GuideCopy> = {
  ja: {
    ...
    startSheetBody: "音声はあとからONにできます。気になる場所だけ候補として残し、記録にするものはあとで選べます。",
    startOutcomeTitle: "成果サンプル",
    startOutcomeSample: "候補: 水辺の草地で、湿った地面と低い葉が見えます。次は葉の裏や水際を近くで見ると、観察のヒントが増えます。",
    startOutcomeBenefit: "許可するまでカメラとマイクは起動しません。候補表示だけで、記録として残すかはあとで選びます。音声は初期OFFで、声らしい音は保存候補から外すよう処理します。",
    startPermissionTitle: "開始前の許可",
    startPermissionBody: "OSの確認画面では、まずカメラだけ許可すれば始められます。自然音を使う時だけ、あとからマイクをONにできます。",
    startFieldRiskNote: "電波が弱い時は端末に一時保存し、容量が少ない時は映像頻度を下げます。長く歩く時は30分ごとに電池と通信を確認してください。",
    ...
    photoFallbackHint: "記録用の写真を選ぶと、ライブ映像なしでガイド解析できます。",
    ...
    frameNotice: "端末画像全体のアップロードボタンはありません。解析用の小さなフレームだけを送り、候補として残るのはサムネイル・解析結果・保存/除外理由です。自然音はONにした時だけ短い候補を別記録にします。",
    naturalSoundBadge: "音声は初期OFF",
    voiceExcludedNotice: "人声の可能性がある音を除外しました",
    audioOffNotice: "音声記録はOFFです。野外らしい発見だけを候補として残し、室内・人物中心・自然手がかりが弱いものは残しません。",
    recordBridge: "見つけたものは、あとから自分の記録として残せます。公開記録には正確な移動軌跡を出しません。",
...
function guideAudioPipelineCopy(lang: SiteLang): GuideAudioPipelineCopy {
  if (lang === "ja") {
    return {
      title: "音声ONで、録った音が研究まで届きます",
      body: "その場では鳴き声メモとして見えます。裏側では「残す → 整理する → 人が確かめる → 研究に渡す」の順で処理されます。",
      aria: "音声データの接続先",
      stages: [
        { label: "残す", body: "人の声らしい音は保存せず、鳥・虫・水音などの短い候補だけ残す" },
        { label: "整理する", body: "似た音をひとまとめにして、あとで代表の音を聞けるようにする" },
        { label: "人が確かめる", body: "AIの候補を確定名にせず、代表の音と確からしさを人のレビューへ回す" },
        { label: "研究に渡す", body: "時刻・場所・観察の手間・公開範囲をそろえ、研究で使える観測データにする" },
      ],
    };
  }
  if (lang === "es") {
