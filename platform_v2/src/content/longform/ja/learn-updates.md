...
- **記録へのフィードバックを追加**: 写真や記録内容に応じて、次に見るとよい点や残すと使える情報を返しやすくしました。
...
- **エリアの記憶を再設計**: 同じ場所の写真や記録を、アルバムのように確認して再訪につなげやすくしました。
- **生物多様性バッジを追加**: 地図上のエリアで、記録数や見どころを直感的に見られる表示を追加しました。
...
**エリアへの記録が見えるようにし、トップページを軽く**

- **記録フィードバックを追加**: ある場所への記録が、そのエリアの理解にどう使えるかを返しやすくしました。
...
- **投稿後の受け取り表示を追加**: 記録がどう形になったか、次に何を残すとよいかを返す準備を進めました。
...
- **公開面の安全性を強化**: お問い合わせの連続送信を制限し、ブラウザ側の保護ヘッダーを追加して、公開サイトの保護を強化しました。
...
- **定点ページを追加**: 同じ場所の再記録をまとめ、写真・動画・環境変化・管理行為を時系列で確認できる入口を用意しました。
...
- **アップデート履歴を復元**: `/learn/updates` が短い要約だけになっていたため、旧本番の v0.1.0〜v0.10.1 の履歴を読み物として戻しました。
- **入口の言葉を整理**: 「AIがすごい」ではなく、散歩中に気になったものを確かめ、あとで開ける記録に残すための説明に寄せました。
...
- **スキャン後の結果シート**: 検出種、確信度、音声クリップを一画面で確認し、必要なものを記録へ昇格しやすくしました。
...
- **バッジ・スコア本格稼働**: 記録数、同定記録、連続投稿などに応じた達成表示を追加しました。
