...
cp "${CANDIDATE_CONFIG}" "${LIVE_AVAILABLE}"
cp "${CANDIDATE_CONFIG}" "${LIVE_ENABLED}"
nginx -t

if [[ "${DRY_RUN}" == "1" ]]; then
  echo "dry-run ok"
  echo "mode: archive-ready"
  echo "public runtime target: v2 (127.0.0.1:3200)"
  echo "legacy php role: rollback/archive artifact only"
  exit 0
fi

SNAPSHOT_PATH="$("${SCRIPT_DIR}/snapshot_cutover_state.sh" "${LABEL}")"
cp "${CANDIDATE_CONFIG}" "${LIVE_AVAILABLE}"
cp "${CANDIDATE_CONFIG}" "${LIVE_ENABLED}"
nginx -t
