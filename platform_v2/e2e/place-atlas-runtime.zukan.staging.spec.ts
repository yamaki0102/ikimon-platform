...
function parseExpectedMapHashes(): Record<string, string> {
  const raw = process.env.ZUKAN_EXPECTED_MAP_SHA256_BY_PATH;
  if (!raw) throw new Error("ZUKAN_EXPECTED_MAP_SHA256_BY_PATH is required");
  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
    throw new Error("ZUKAN_EXPECTED_MAP_SHA256_BY_PATH must be valid JSON");
  }
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("ZUKAN_EXPECTED_MAP_SHA256_BY_PATH must be an object");
  }
...
async function installRuntimeFixtures(page: Page, profile: Record<string, unknown>, mutations: string[]): Promise<void> {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (!SAFE_METHODS.has(method)) {
      ...
      await fulfillJson(route, 409, { ok: false, error: "zukan_runtime_unknown_mutation_rejected" });
      return;
    }
...
async function openProfile(browser: Browser, localePath: string, locale: string, profile: Record<string, unknown>): Promise<RuntimeSession> {
  ...
  const url = `${localePath}?tab=places&lng=138.3805&lat=34.9702&z=16.4&zukan_runtime_qa=${encodeURIComponent(process.env.IKIMON_EXPECTED_GIT_SHA ?? "unknown")}`;
...
test.describe.serial("ZUKAN Place Atlas exact staging runtime", () => {
  for (const locale of LOCALES) {
    test(`timeline is materialization-bound and localized (${locale.path})`, async ({ browser }) => {
      ...
      session.mutationEvents.length = 0;
      await cta.evaluate((node) => {
        node.addEventListener("click", (event) => event.preventDefault(), { capture: true, once: true });
      });
      await cta.click();
      await expect.poll(() => session.mutationEvents.filter((event) => event === `POST ${KPI_PATH}`).length).toBe(1);
