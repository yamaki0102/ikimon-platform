...
const canonicalAiSubjectScenePath =
  "/ja/observations/record-1778549526406?subject=occ%3Arecord-1778549526406%3A0";
...
test.describe("production candidate smoke", () => {
  ...
  test("canonical scene renders visible AI subjects as observation records", async ({ page }) => {
    test.skip(
      !process.env.PRODUCTION_SMOKE_BASE_URL?.trim(),
      "requires a production candidate base URL or SSH tunnel",
    );

    const response = await page.goto(canonicalAiSubjectScenePath, { waitUntil: "domcontentloaded" });
    ...
    for (const name of ["セイヨウミツバチ", "イネ科の一種"]) {
      ...
      await expect(card, `${name} is an occurrence-backed observation record`).toHaveAttribute(
        "href",
        /[?&]subject=occ%3Arecord-1778549526406%3A\d+/,
      );
      await expect(card, `${name} keeps the AI-only observation-record boundary visible`).toContainText(
        "AIが写真から分けた観測レコード",
      );
    ...
    await recordSmokeCheckpoint("canonical_scene_ai_subject_records", {
      path: canonicalAiSubjectScenePath,
