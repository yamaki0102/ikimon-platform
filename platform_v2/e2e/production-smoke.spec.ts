...
import { request as playwrightRequest, test, expect, type APIRequestContext, type BrowserContext, type Page } from "@playwright/test";
...
type ProductionSmokeCleanupPayload = JsonPayload & {
  deleted?: { r2Objects?: unknown };
};
...
const productionSmokeRunPrefix = process.env.PRODUCTION_SMOKE_UI_PREFIX?.trim()
  || `smoke-ui-${Date.now().toString(36)}-${randomUUID().replaceAll("-", "").slice(0, 16)}`;
let productionSmokeOwnedFixtureCreated = false;
let productionSmokeCleanupToken = "";

function productionSmokePrefix(): string {
  return productionSmokeRunPrefix;
}
...
function requireProductionSmokeWriteScope(scope: ProductionSmokeWriteScope): void {
  ...
  if (!process.env.V2_PRIVILEGED_WRITE_API_KEY?.trim()) {
    throw new Error("production_smoke_requires_privileged_write_key_for_cleanup");
  }
  productionSmokeOwnedFixtureCreated = true;
}

async function cleanupCloudflareSmokeFixtures(): Promise<ProductionSmokeCleanupPayload> {
  ...
  try {
    const response = await api.post("/api/v1/internal/production-smoke/cleanup", {
      headers: {
        accept: "application/json",
        "content-type": "application/json",
        "x-ikimon-write-key": writeKey,
        ...(stagingBasicAuthHeader() ? { authorization: stagingBasicAuthHeader()! } : {}),
      },
      data: { fixturePrefix: productionSmokeRunPrefix, cleanupToken: productionSmokeCleanupToken },
    });
    const payload = await response.json().catch(() => null) as ProductionSmokeCleanupPayload | null;
    expect(response.ok(), String(payload?.error ?? "production_smoke_cloudflare_cleanup_failed")).toBeTruthy();
    return payload ?? {};
  } finally {
    await api.dispose();
  }
...
async function startCloudflareSmokeRun(): Promise<void> {
  const baseUrl = productionSmokeBaseUrl();
  const writeKey = process.env.V2_PRIVILEGED_WRITE_API_KEY!.trim();
  const api = await playwrightRequest.newContext({ baseURL: baseUrl, ignoreHTTPSErrors: true });
  try {
    const response = await api.post("/api/v1/internal/production-smoke/runs", {
      headers: {
        accept: "application/json",
        "content-type": "application/json",
        "x-ikimon-write-key": writeKey,
      ...
      data: { fixturePrefix: productionSmokeRunPrefix },
    });
    const payload = await response.json().catch(() => null) as JsonPayload | null;
    expect(response.ok(), String(payload?.error ?? "production_smoke_run_start_failed")).toBeTruthy();
    expect(payload?.cleanupToken, "production smoke run must issue an owned cleanup token").toBeTruthy();
    productionSmokeCleanupToken = String(payload!.cleanupToken);
  } finally {
    await api.dispose();
  }
}
...
test.describe("production candidate smoke", () => {
  test.beforeAll(async () => {
    if (!productionSmokeWriteScope()) return;
    await startCloudflareSmokeRun();
  });

  test.afterAll(async () => {
    test.setTimeout(120_000);
    if (!productionSmokeOwnedFixtureCreated) return;
    const cleanup = await cleanupCloudflareSmokeFixtures();
    expect(cleanup.cleanupZero, "production smoke D1/R2 cleanup must leave zero residue").toBe(true);
    await recordSmokeCheckpoint("cloudflare_fixture_cleanup_zero", {
      fixturePrefix: productionSmokeRunPrefix,
      deletedR2Objects: cleanup.deleted?.r2Objects,
    });
  });
  ...
  test("[shared-production-write] identification workbench saves reference evidence against the production candidate", async ({ browser }) => {
    ...
    try {
      ...
      const fixtureVisitId = `${prefix}-identification-target`;
      const fixtureResponse = await identifierContext.request.post(joinUrl(baseUrl, "/api/v1/observations/upsert"), {
        headers: jsonHeaders(baseUrl, identifier),
        data: {
          observationId: fixtureVisitId,
          clientSubmissionId: `${fixtureVisitId}-${Date.now()}`,
          ...
          note: `production identification smoke owned fixture ${prefix}`,
          taxon: { vernacularName: "同定待ち", scientificName: null, rank: "unknown" },
          sourcePayload: {
      ...
      const fixturePayload = await jsonFromResponse(fixtureResponse, "owned identification fixture");
      expect(fixtureResponse.ok(), String(fixturePayload.error ?? "owned_identification_fixture_failed")).toBeTruthy();
      const occurrenceId = String(fixturePayload.occurrenceId ?? `occ:${fixtureVisitId}:0`);
      ...
      const card = page.locator(`[data-records-identify-card][data-identify-endpoint*="${encodeURIComponent(occurrenceId)}"]`);
      await expect(card, "production needs_id workbench should expose only the smoke-owned candidate target").toHaveCount(1);
      ...
      expect(occurrenceIdFromIdentificationEndpoint(identifyEndpoint!)).toBe(occurrenceId);
      ...
      expect(visitId).toBe(fixtureVisitId);
