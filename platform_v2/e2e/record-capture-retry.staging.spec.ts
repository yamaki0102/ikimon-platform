...
test.describe.serial("capture P0 failure, retry, and success stay fixture-only", () => {
  test("camera path saves one Record, retries media, and reaches success without a second Record", async ({ page }, testInfo) => {
    ...
    const response = await page.goto(`/ja/record?userId=${encodeURIComponent(`${fixtureId}-user`)}`, {
