...
async function fixture(t, { leak = false, responseSha = SHA, switchEnd = false } = {}) {
  let identityCalls = 0;
  const server = createServer((req, res) => {
    const headers = {
      ...
      'x-ikimon-worker-version': 'test-version-1',
      'x-ikimon-schema-digest': 'schema-test-1'
    };
...
test('missing secrets are BLOCKED before probes run', async (t) => {
  const baseUrl = await fixture(t);
