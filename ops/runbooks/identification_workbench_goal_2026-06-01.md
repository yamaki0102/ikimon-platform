...
## 2026-06-11 Claude UX review とP0反映

レビュー証拠:

- `claude-fable-5`: 2回試行したが、Claude Code API が policy refusal 文を返したため内容レビューとして不採用。
- `claude-opus-4-8`: 成功。raw review は `E:\Projects\_agent_scratch\claude-latest-review\ikimon-identification-ux-20260611-opus\claude-review-20260611-opus.md`。
- `sonnet`: 先に一度取得したが、Fable拒否後の正しいフォールバックはOpusだったため、最終判断ではOpusレビューを正にした。

採用したP0:

- AI由来の候補を人の同定と混同させないため、確認待ちカードと同定パネルの候補ラベルを `AI候補` / `AI suggestion` 系に変更。
- 画面内の全カードを処理済みにした時、最後の記録を表示し続けず、同定パネルを空状態へ切り替える。
- 所有済み資料を自動チェックしない。`持っている資料` と `実際に確認した資料` を分け、Tier 3 根拠が自動生成されないようにする。
- AI候補名が入力欄に入っている時は、`AIの候補です` の注意を表示し、利用者が編集したら注意を隠す。

## 2026-06-11 Opus改善案の追加実装

追加で実装したもの:

- server-side hold: `identification_workbench_holds` を追加し、利用者ごとに保留した確認待ちレコードを永続化する。`/api/v1/observations/:id/identification-workbench-hold` はログイン必須、rate limit 対象。
- `needs_id` の追加ページング: `/api/v1/records/needs-id-page` を追加し、同定ワークベンチでも遅延読み込みを使えるようにした。追加カードは `records:lazy-appended` で同定パネルへ通知する。
- 保留済み除外: `/records?view=needs_id` と追加ページングの両方で、自分が保留した occurrence をキューから外す。
- 資料なし保存の明示: `support` 保存時は資料選択か `資料なしで保存する` の明示チェックを必須にし、無意識の Tier 3 根拠化を避ける。
- 保存失敗のUI: 保存系の通信失敗時に汎用の `保存できませんでした。少し時間をおいてもう一度試してください。` を出す。
- 連続処理キーボード: 入力欄以外で `J` 次へ、`K` 前へ、`1` 支持、`2` 別候補、`3` 証拠不足、`4` 保留を使えるようにした。
- カード選択時のスクロール: モバイルでパネルが画面外の時だけ、選択後に同定パネルへ寄せる。

検証:

- `npx tsx --test src/routes/publicCopy.routes.test.ts`
- `npx tsx --test src/routes/publicCopy.routes.test.ts src/routes/identification.write.routes.test.ts`
- `npm run typecheck`
- `git diff --check -- platform_v2/src/routes/read.ts platform_v2/src/routes/write.ts platform_v2/src/services/identificationWorkbenchHolds.ts platform_v2/db/migrations/0118_identification_workbench_holds.sql platform_v2/src/routes/publicCopy.routes.test.ts platform_v2/src/routes/identification.write.routes.test.ts platform_v2/e2e/identification-workbench.staging.spec.ts ops/runbooks/identification_workbench_goal_2026-06-01.md`
- `powershell -ExecutionPolicy Bypass -File E:\Projects\00_all_projects_management\scripts\run_playwright.ps1 -ScriptPath E:\Projects\_agent_scratch\yamaki0102-ikimon-platform\ikimon-identification-ux-20260611\verify-identification-workbench.cjs`
- PC screenshot: `E:\Projects\_agent_scratch\yamaki0102-ikimon-platform\ikimon-identification-ux-20260611\identification-workbench-desktop.png`
- mobile screenshot: `E:\Projects\_agent_scratch\yamaki0102-ikimon-platform\ikimon-identification-ux-20260611\identification-workbench-mobile.png`

未実施:

- staging / production deploy は未実施。repo に同定以外の dirty が多く、deploy lane 分離なしに進めない。
