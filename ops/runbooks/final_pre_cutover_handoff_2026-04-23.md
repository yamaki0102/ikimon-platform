...
## 現在の既知 BLOCKER (2026-04-23 16:26 JST)

`/ops/readiness` スナップショット:
- `gates.parityVerified: false` 🟡
- `gates.rollbackSafetyWindowReady: false` 🟡

**原因**: `latestDriftReport.finished_at` = 2026-04-22 14:42 JST（24.7h 前）で stale。
**解消**: T-24h の最初のタスクで `npm run report:legacy-drift` を再実行すれば fresh 化 → 全 GREEN 復帰見込。

## 時系列タスク

### T-24h (前日、推定所要 30-45分)

#### 1. drift report 再実行 (STALE 解消) — **最優先**
...
ref: [`ops/CUTOVER_RUNBOOK.md` §カットオーバー前に必要な環境変数追加](../CUTOVER_RUNBOOK.md#カットオーバー前に必要な環境変数追加)
...
**成功判定**:
```bash
curl -s http://127.0.0.1:3201/ops/readiness | jq '.gates.audioArchiveReady'
# => true
pm2 describe ikimon-v2-production-api | grep -E 'GEMINI|CLOUDFLARE'
```
