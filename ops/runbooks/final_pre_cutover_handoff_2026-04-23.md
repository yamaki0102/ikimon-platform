...
### Known Limitation — 2026-04-23 17:10 JST すべて解消

| 項目 | 状態 | 対処タイミング |
|---|---|---|
| ~~migration 0020 (audio) 未適用~~ | ✅ 2026-04-23 17:10 本番 DB 適用済 (sudo postgres 経由) | — |
| ~~`specialist_authorities` テーブル不在~~ | ✅ 同日 migration 0018 で作成済 (行 0 で authorityFill 100% 扱い) | — |
| ~~`CLOUDFLARE_STREAM_*` env 未設定~~ | ✅ 本番 v2 pm2 env に既設定 | — |
| 本番 v2 dist 反映 | ⏳ dist が古く gates.audioArchiveReady キーが旧フォーマット | cutover 後 T+30m に `npm run build` + `pm2 restart --update-env` |
