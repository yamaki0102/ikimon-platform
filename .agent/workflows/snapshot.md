...
## 重要情報
- **ローカル git remote**: `yamaki0102/antigravity`（モノリポ）
- **ikimon コード専用リポ**: `yamaki0102/ikimon-platform`
- **Google Drive 上で git 操作する場合、`index.lock` が同期プロセスに掌まれて失敗することがある**
...
- `ikimon-platform` リポはローカルの `antigravity` モノリポとは**別のリポジトリ**
...
## 🚨 よくあるミス
1. **Google Drive が `index.lock` を掌む** → 同期完了を待つか、API フォールバックを使う
